...
规则 ID：`srd-5.1:will-o-wisp`。结论：**本次已覆盖范围通过**。
...
通过：电击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 鬼火 使用电击攻击 验证目标：电击 → 验证目标 3+4=7 未命中；共造成 0 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 电击 / `shock` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 隐形 / `invisibility` | headless | invisibility | 通过 | minimum、maximum、no-resource |
| 动作 | Consume Life / `consume-life` | headless | saving-throw-terminal-effect | 通过 | minimum、maximum、no-resource、out-of-range、source-blinded、target-not-zero |
...
| --- | --- | --- | --- | --- |
| 吞噬生命 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 虚体短暂 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 虚体移动 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 可变照明 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/will-o-wisp.json)
- [总索引与运行说明](../README.md)
