...
规则 ID：`srd-5.1:pony`。结论：**本次已覆盖范围通过**。
...
通过：蹄击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 310/320。

> 矮种马 使用蹄击攻击 验证目标：蹄击 → 验证目标 14+4=18 命中；共造成 10 点伤害。
