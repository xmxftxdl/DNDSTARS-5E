...
规则 ID：`srd-5.1:giant-ape`。结论：**本次已覆盖范围通过**。
...
通过：拳击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 296/320。

> 巨猿 使用拳击攻击 验证目标：拳击 → 验证目标 2+9=11 命中；共造成 24 点伤害。
...
- [本怪物字段与结果证据](../evidence/giant-ape.json)
- [总索引与运行说明](../README.md)
