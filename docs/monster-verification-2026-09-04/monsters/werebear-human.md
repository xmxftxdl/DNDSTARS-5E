...
规则 ID：`srd-5.1:werebear-human`。结论：**本次已覆盖范围通过**。
...
通过：巨斧。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 308/320。

> 熊人（人类形态） 使用巨斧攻击 验证目标：巨斧 → 验证目标 9+7=16 命中；共造成 12 点伤害。
...
- [本怪物字段与结果证据](../evidence/werebear-human.json)
- [总索引与运行说明](../README.md)
