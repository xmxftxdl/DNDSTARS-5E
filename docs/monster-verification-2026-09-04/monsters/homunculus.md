...
规则 ID：`srd-5.1:homunculus`。结论：**本次已覆盖范围通过**。

## 已知问题与边界

- 已修复：啃咬不再因“豁免失败 5 点或更多”而整体降级。命中后会自动结算 DC 10 体质豁免；普通失败中毒 1 分钟，深度失败额外掷 1d10 分钟，并让中毒与昏迷共享该持续时间。昏迷依赖中毒，受伤或被唤醒会结束。7073/7074 双端真实 UI 已验证掷出 5 分钟后写入 50 回合的两种状态、1 点伤害和双端日志。
...
- 通过：7073/7074 双端真实页面完成命中、深度失败豁免和额外持续时间结算。目标 HP 20 → 19；持续时间骰为 5 分钟，中毒与昏迷均写入 50 回合，昏迷包含受伤／唤醒中断并依赖中毒，DM 与玩家日志一致。

> 魔宠人偶 使用啃咬攻击 深度失败验证目标：啃咬 → 深度失败验证目标 10+4=14 命中；共造成 1 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 啃咬 / `bite` | headless | weapon-attack | 通过 | miss、hit、critical、bite-poisoned-unconscious:apply、bite-poisoned-unconscious:save、bite-poisoned-unconscious:condition-immune |
...
- [本怪物字段与结果证据](../evidence/homunculus.json)
- [啃咬毒素真实 UI 证据](../evidence/homunculus-bite-ui.json)
- [总索引与运行说明](../README.md)
