...
规则 ID：`srd-5.1:mimic`。结论：**本次已覆盖范围通过**。
...
通过：伪足。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 315/320。

> 拟态怪 使用伪足攻击 验证目标：伪足 → 验证目标 9+5=14 命中；共造成 5 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 伪足 / `pseudopod` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 啃咬 / `bite` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 伪足 (Adhesive Object Form) / `pseudopod-adhesive-object-form` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| 黏附（仅物体形态） | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 虚假外表（仅物体形态） | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 擒抱者 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/mimic.json)
- [总索引与运行说明](../README.md)
