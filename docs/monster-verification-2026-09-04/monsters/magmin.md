...
规则 ID：`srd-5.1:magmin`。结论：**本次已覆盖范围通过**。
...
通过：触碰。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 314/320。

> 岩浆怪 使用触碰攻击 验证目标：触碰 → 验证目标 18+4=22 命中；共造成 6 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 触碰 / `touch` | headless | weapon-attack | 通过 | miss、hit、critical、ignited:apply |
...
| 燃火照明 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/magmin.json)
- [总索引与运行说明](../README.md)
