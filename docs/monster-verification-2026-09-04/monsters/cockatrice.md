...
规则 ID：`srd-5.1:cockatrice`。结论：**本次已覆盖范围通过**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 318/320。

> 鸡蛇兽 使用啃咬攻击 验证目标：啃咬 → 验证目标 14+3=17 命中；共造成 2 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 啃咬 / `bite` | headless | weapon-attack | 通过 | miss、hit、critical、bite-petrification:apply、bite-petrification:save、bite-petrification:condition-immune |
...
- [本怪物字段与结果证据](../evidence/cockatrice.json)
- [总索引与运行说明](../README.md)
