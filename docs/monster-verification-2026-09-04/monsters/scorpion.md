...
规则 ID：`srd-5.1:scorpion`。结论：**本次已覆盖范围通过**。
...
通过：螫刺。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 317/320。

> 蝎子 使用螫刺攻击 验证目标：螫刺 → 验证目标 16+2=18 命中；共造成 3 点伤害。
...
- [本怪物字段与结果证据](../evidence/scorpion.json)
- [总索引与运行说明](../README.md)
