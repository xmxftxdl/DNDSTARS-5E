...
规则 ID：`srd-5.1:bandit-captain`。结论：**本次已覆盖范围通过**。
...
通过：弯刀。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 314/320。

> 强盗头目 使用弯刀攻击 验证目标：弯刀 → 验证目标 13+5=18 命中；共造成 6 点伤害。
...
| 动作 | 匕首 / `dagger` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 多重攻击：匕首 ×2 / `multiattack-daggers-ranged` | headless | multiattack | 通过 | complete-sequence |
| 反应 | 招架 / `parry` | headless | parry | 通过 | SRD monster Parry in Headless combat applies the exact +2 AC reaction for 'bandit-captain' |
...
- [本怪物字段与结果证据](../evidence/bandit-captain.json)
- [总索引与运行说明](../README.md)
