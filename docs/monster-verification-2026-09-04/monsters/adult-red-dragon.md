...
规则 ID：`srd-5.1:adult-red-dragon`。结论：**本次已覆盖范围通过**。
...
> 成年红龙 使用啃咬攻击 验证目标：啃咬 → 验证目标 14+14=28 命中；共造成 27 点伤害。

### 传奇与特殊动作专项

- 通过：侦测（场景 0）；DM 日志：成年红龙使用传奇动作侦测：检定 20。
- 通过：翼击（消耗 2 动作）（场景 1）；DM 日志：成年红龙使用传奇动作翼击（消耗 2 动作）。
- 通过：翼击（消耗 2 动作）（场景 2）；DM 日志：成年红龙使用传奇动作翼击（消耗 2 动作）。
...
- [本怪物字段与结果证据](../evidence/adult-red-dragon.json)
- [总索引与运行说明](../README.md)
