...
规则 ID：`srd-5.1:guardian-naga`。结论：**本次已覆盖范围通过**。
...
> 守护纳迦 使用啃咬攻击 验证目标：啃咬 → 验证目标 12+8=20 命中；共造成 36 点伤害。
...
| 动作 | 喷吐毒液 / `spit-poison` | headless | weapon-attack | 通过 | miss、hit、critical |
...
- [本怪物字段与结果证据](../evidence/guardian-naga.json)
- [总索引与运行说明](../README.md)
