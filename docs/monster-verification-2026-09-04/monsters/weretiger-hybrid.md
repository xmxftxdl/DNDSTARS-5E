# 虎人（混合形态）（Weretiger, Hybrid Form）

规则 ID：`srd-5.1:weretiger-hybrid`。结论：**本次已覆盖范围通过**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 312/320。

> 虎人（混合形态） 使用啃咬攻击 验证目标：啃咬 → 验证目标 13+5=18 命中；共造成 8 点伤害。
...
| 动作 | 弯刀 / `scimitar` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 长弓 / `longbow` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 多重攻击：弯刀 ×2 / `multiattack-scimitar` | headless | multiattack | 通过 | complete-sequence |
| 动作 | 多重攻击：长弓 ×2 / `multiattack-longbow` | headless | multiattack | 通过 | complete-sequence |
| 附赠动作 | 猛扑：啃咬 / `pounce-bite-bonus-action` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| 猛扑 | headless | charge-damage | 通过 | no-movement、below-threshold、at-threshold |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/weretiger-hybrid.json)
- [总索引与运行说明](../README.md)
