...
规则 ID：`srd-5.1:flying-sword`。结论：**本次已覆盖范围通过**。
...
通过：长剑。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 飞剑 使用长剑攻击 验证目标：长剑 → 验证目标 2+3=5 未命中；共造成 0 点伤害。
...
- [本怪物字段与结果证据](../evidence/flying-sword.json)
- [总索引与运行说明](../README.md)
