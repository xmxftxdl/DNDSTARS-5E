...
规则 ID：`srd-5.1:manticore`。结论：**本次已覆盖范围通过**。
...
> 蝎尾狮 使用啃咬攻击 验证目标：啃咬 → 验证目标 16+5=21 命中；共造成 4 点伤害。
...
| 动作 | 尾刺 / `tail-spike` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 多重攻击：尾刺 ×3 / `multiattack-tail-spikes` | headless | multiattack | 通过 | complete-sequence |
...
| --- | --- | --- | --- | --- |
| 尾刺再生 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/manticore.json)
- [总索引与运行说明](../README.md)
