...
规则 ID：`srd-5.1:ettin`。结论：**本次已覆盖范围通过**。
...
通过：战斧。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 306/320。

> 双头巨人 使用战斧攻击 验证目标：战斧 → 验证目标 18+7=25 命中；共造成 14 点伤害。
...
| 动作 | 战斧 / `battleaxe` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| --- | --- | --- | --- | --- |
| 双头 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 警醒 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/ettin.json)
- [总索引与运行说明](../README.md)
