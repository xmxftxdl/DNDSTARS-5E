...
规则 ID：`srd-5.1:storm-giant`。结论：**本次已覆盖范围通过**。
...
通过：巨剑。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 295/320。

> 风暴巨人 使用巨剑攻击 验证目标：巨剑 → 验证目标 10+14=24 命中；共造成 25 点伤害。
...
| 动作 | 闪电打击 / `lightning-strike` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target、exhausted-resource |
...
- [本怪物字段与结果证据](../evidence/storm-giant.json)
- [总索引与运行说明](../README.md)
