...
规则 ID：`srd-5.1:vulture`。结论：**本次已覆盖范围通过**。
...
通过：喙击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 317/320。

> 秃鹫 使用喙击攻击 验证目标：喙击 → 验证目标 8+2=10 命中；共造成 3 点伤害。
...
- [本怪物字段与结果证据](../evidence/vulture.json)
- [总索引与运行说明](../README.md)
