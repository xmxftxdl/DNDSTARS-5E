...
规则 ID：`srd-5.1:pit-fiend`。结论：**本次已覆盖范围通过**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 299/320。

> 深狱炼魔 使用啃咬攻击 验证目标：啃咬 → 验证目标 15+14=29 命中；共造成 21 点伤害。
...
| 动作 | 啃咬 / `bite` | headless | weapon-attack | 通过 | miss、hit、critical、bite-poison:apply、bite-poison:save |
| 动作 | 爪击 / `claw` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 硬头锤 / `mace` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| --- | --- | --- | --- | --- |
| 恐惧灵光 | headless | turn-start-saving-throw-aura | 通过 | successful-save、failed-save、outside-range |
...
- [本怪物字段与结果证据](../evidence/pit-fiend.json)
- [总索引与运行说明](../README.md)
