...
规则 ID：`srd-5.1:quasit`。结论：**本次已覆盖范围通过**。
...
通过：爪击（野兽形态为啃咬）。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 316/320。

> 夸塞魔 使用爪击（野兽形态为啃咬）攻击 验证目标：爪击（野兽形态为啃咬） → 验证目标 8+4=12 命中；共造成 4 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 爪击（野兽形态为啃咬） / `claw-bite-in-beast-form` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 惊吓 / `scare` | headless | saving-throw-condition | 通过 | minimum、maximum、no-resource、out-of-range |
...
- [本怪物字段与结果证据](../evidence/quasit.json)
- [总索引与运行说明](../README.md)
