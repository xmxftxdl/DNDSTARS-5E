...
规则 ID：`srd-5.1:werewolf-human`。结论：**本次已覆盖范围通过**。
...
通过：长矛。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 309/320。

> 狼人（人类形态） 使用长矛攻击 验证目标：长矛 → 验证目标 20+4=24 命中；共造成 11 点伤害。
...
- [本怪物字段与结果证据](../evidence/werewolf-human.json)
- [总索引与运行说明](../README.md)
