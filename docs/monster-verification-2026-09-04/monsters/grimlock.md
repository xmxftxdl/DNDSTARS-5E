...
规则 ID：`srd-5.1:grimlock`。结论：**本次已覆盖范围通过**。
...
通过：尖刺骨棒。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 格里姆洛克 使用尖刺骨棒攻击 验证目标：尖刺骨棒 → 验证目标 2+5=7 未命中；共造成 0 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 尖刺骨棒 / `spiked-bone-club` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| --- | --- | --- | --- | --- |
| 盲感官 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/grimlock.json)
- [总索引与运行说明](../README.md)
