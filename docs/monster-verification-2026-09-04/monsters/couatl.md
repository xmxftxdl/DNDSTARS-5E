...
规则 ID：`srd-5.1:couatl`。结论：**Headless 已验证，仍有 DM 裁定边界**。
...
> 羽蛇 使用啃咬攻击 验证目标：啃咬 → 验证目标 17+8=25 命中；共造成 11 点伤害。
...
| 动作 | 缠勒 / `constrict` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| 心灵屏障 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/couatl.json)
- [总索引与运行说明](../README.md)
