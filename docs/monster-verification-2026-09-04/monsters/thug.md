...
规则 ID：`srd-5.1:thug`。结论：**本次已覆盖范围通过**。
...
通过：硬头锤。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 315/320。

> 暴徒 使用硬头锤攻击 验证目标：硬头锤 → 验证目标 16+4=20 命中；共造成 5 点伤害。
