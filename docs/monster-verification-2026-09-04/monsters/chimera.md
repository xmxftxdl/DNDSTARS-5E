...
规则 ID：`srd-5.1:chimera`。结论：**本次已覆盖范围通过**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 309/320。

> 奇美拉 使用啃咬攻击 验证目标：啃咬 → 验证目标 4+7=11 命中；共造成 11 点伤害。
...
| 动作 | 多重攻击：火焰吐息 ×3 / `multiattack-fire-breath-instead-of-bite` | headless | multiattack | 通过 | complete-sequence |
| 动作 | 多重攻击：啃咬 ×3 / `multiattack-fire-breath-instead-of-horns` | headless | multiattack | 通过 | complete-sequence |
...
- [本怪物字段与结果证据](../evidence/chimera.json)
- [总索引与运行说明](../README.md)
