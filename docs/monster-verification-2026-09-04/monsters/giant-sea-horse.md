# 巨型海马（Giant Sea Horse）

规则 ID：`srd-5.1:giant-sea-horse`。结论：**本次已覆盖范围通过**。
...
通过：冲撞。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 318/320。

> 巨型海马 使用冲撞攻击 验证目标：冲撞 → 验证目标 9+3=12 命中；共造成 2 点伤害。
...
- [本怪物字段与结果证据](../evidence/giant-sea-horse.json)
- [总索引与运行说明](../README.md)
