...
规则 ID：`srd-5.1:acolyte`。结论：**本次已覆盖范围通过**。
...
通过：木棒。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 侍僧 使用木棒攻击 验证目标：木棒 → 验证目标 6+2=8 未命中；共造成 0 点伤害。
...
- [本怪物字段与结果证据](../evidence/acolyte.json)
- [总索引与运行说明](../README.md)
