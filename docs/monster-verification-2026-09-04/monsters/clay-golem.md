...
规则 ID：`srd-5.1:clay-golem`。结论：**本次已覆盖范围通过**。
...
通过：猛击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 301/301。

> 黏土魔像 使用猛击攻击 验证目标：猛击 → 验证目标 6+8=14 命中；共造成 19 点伤害。
...
| 动作 | 加速 / `haste` | headless | self-combat-buff | 通过 | minimum、maximum、no-resource |
| 附赠动作 | 猛击 (加速) / `haste-slam-bonus-action` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| --- | --- | --- | --- | --- |
| 强酸吸收 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 狂暴失控 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/clay-golem.json)
- [总索引与运行说明](../README.md)
