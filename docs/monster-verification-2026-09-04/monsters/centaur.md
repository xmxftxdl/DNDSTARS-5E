...
规则 ID：`srd-5.1:centaur`。结论：**本次已覆盖范围通过**。
...
通过：长枪。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 313/320。

> 半人马 使用长枪攻击 验证目标：长枪 → 验证目标 19+6=25 命中；共造成 7 点伤害。
...
| 动作 | 长枪 / `pike` | headless | weapon-attack | 通过 | miss、hit、critical |
...
- [本怪物字段与结果证据](../evidence/centaur.json)
- [总索引与运行说明](../README.md)
