...
规则 ID：`srd-5.1:medusa`。结论：**本次已覆盖范围通过**。
...
通过：蛇发。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 304/320。

> 美杜莎 使用蛇发攻击 验证目标：蛇发 → 验证目标 17+5=22 命中；共造成 16 点伤害。
...
| 动作 | 蛇发 / `snake-hair` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| --- | --- | --- | --- | --- |
| 石化凝视 | headless | turn-start-gaze | 通过 | successful-save、failed-save、severe-failure、outside-range、avert-eyes |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/medusa.json)
- [总索引与运行说明](../README.md)
