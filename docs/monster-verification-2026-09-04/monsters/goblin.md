...
规则 ID：`srd-5.1:goblin`。结论：**本次已覆盖范围通过**。
...
通过：弯刀。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 313/320。

> 地精 使用弯刀攻击 验证目标：弯刀 → 验证目标 9+4=13 命中；共造成 7 点伤害。
...
| 动作 | 短弓 / `shortbow` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| --- | --- | --- | --- | --- |
| 灵活逃脱 | headless | nimble-escape | 通过 | SRD monster actions in the D&D 5e Headless engine validates Nimble Escape from the monster stat block and spends a bonus action |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/goblin.json)
- [总索引与运行说明](../README.md)
