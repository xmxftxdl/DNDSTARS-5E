...
规则 ID：`srd-5.1:behir`。结论：**本次已覆盖范围通过**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 278/320。

> 贝希摩斯 使用啃咬攻击 验证目标：啃咬 → 验证目标 20+10=30 命中；共造成 42 点伤害。
...
| 动作 | 闪电吐息 / `lightning-breath` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target、exhausted-resource |
...
- [本怪物字段与结果证据](../evidence/behir.json)
- [总索引与运行说明](../README.md)
