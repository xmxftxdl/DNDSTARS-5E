...
规则 ID：`srd-5.1:ancient-white-dragon`。结论：**本次已覆盖范围通过**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 284/320。

> 远古白龙 使用啃咬攻击 验证目标：啃咬 → 验证目标 2+14=16 命中；共造成 36 点伤害。
...
| 动作 | 寒冰吐息 / `cold-breath` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target、exhausted-resource |
...
| --- | --- | --- | --- | --- |
| 冰面行走 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/ancient-white-dragon.json)
- [总索引与运行说明](../README.md)
