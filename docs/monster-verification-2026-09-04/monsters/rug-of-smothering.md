# 窒息地毯（Rug of Smothering）

规则 ID：`srd-5.1:rug-of-smothering`。结论：**本次已覆盖范围通过**。

## 已知问题与边界

- 已修复：反魔法易感特性原先被怪物自身 schema 拒绝；目录往返解析已通过。

当前仍有 0 个 Headless 动作未被本次目录矩阵完整确认，0 个动作使用 DM 裁定或属于非战斗能力。

## 实际 UI

通过：窒息包裹。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 窒息地毯 使用窒息包裹攻击 验证目标：窒息包裹 → 验证目标 2+5=7 未命中；共造成 0 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 窒息包裹 / `smother` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| --- | --- | --- | --- | --- |
| 反魔法易感 | dm-adjudication | antimagic-susceptibility | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/rug-of-smothering.json)
- [总索引与运行说明](../README.md)
