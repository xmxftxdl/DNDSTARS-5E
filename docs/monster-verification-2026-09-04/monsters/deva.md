...
规则 ID：`srd-5.1:deva`。结论：**Headless 已验证，仍有 DM 裁定边界**。
...
通过：硬头锤。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 296/320。

> 提婆 使用硬头锤攻击 验证目标：硬头锤 → 验证目标 12+8=20 命中；共造成 24 点伤害。
