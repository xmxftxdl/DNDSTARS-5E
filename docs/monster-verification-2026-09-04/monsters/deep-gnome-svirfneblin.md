# 地底侏儒（斯涅布力）（Deep Gnome (Svirfneblin)）

规则 ID：`srd-5.1:deep-gnome-svirfneblin`。结论：**本次已覆盖范围通过**。
...
通过：战镐。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 313/320。

> 地底侏儒（斯涅布力） 使用战镐攻击 验证目标：战镐 → 验证目标 10+4=14 命中；共造成 7 点伤害。
...
| 动作 | 毒镖 / `poisoned-dart` | headless | weapon-attack | 通过 | miss、hit、critical、dart-poisoned:apply、dart-poisoned:save、dart-poisoned:condition-immune |
...
| --- | --- | --- | --- | --- |
| 岩石伪装 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 侏儒狡黠 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/deep-gnome-svirfneblin.json)
- [总索引与运行说明](../README.md)
