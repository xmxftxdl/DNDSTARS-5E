...
规则 ID：`srd-5.1:cloaker`。结论：**Headless 已验证，仍有 DM 裁定边界**。

## 已知问题与边界

- 保留边界：幻象需要在非明亮光照下创建三个分身，并同时拦截攻击与有害法术；分身还要独立使用斗篷怪 AC／豁免并在进入明亮光照时消失。现有镜影拦截器只完整覆盖攻击，尚未覆盖有害法术与光照生命周期，因此不能安全开放整项 Headless。
...
> 披风怪 使用啃咬攻击 验证目标：啃咬 → 验证目标 14+6=20 命中；共造成 13 点伤害。
...
| 动作 | 哀嚎 / `moan` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target |
| 动作 | 幻象 / `phantasms` | dm-adjudication | other | DM 裁定 / 非战斗 | — |
...
| --- | --- | --- | --- | --- |
| 伤害转移 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 虚假外表 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 畏光 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/cloaker.json)
- [总索引与运行说明](../README.md)
