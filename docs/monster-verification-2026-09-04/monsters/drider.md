...
规则 ID：`srd-5.1:drider`。结论：**本次已覆盖范围通过**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 313/320。

> 蛛化精灵 使用啃咬攻击 验证目标：啃咬 → 验证目标 15+6=21 命中；共造成 7 点伤害。
...
| 动作 | 多重攻击：长剑 ×3 / `multiattack-longsword` | headless | multiattack | 通过 | complete-sequence |
| 动作 | 多重攻击：啃咬 ×3 / `multiattack-bite-and-longbow` | headless | multiattack | 通过 | complete-sequence |
| 动作 | 多重攻击：啃咬 ×3 / `multiattack-bite-and-longsword` | headless | multiattack | 通过 | complete-sequence |
...
| --- | --- | --- | --- | --- |
| 精类血统 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 天生施法 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 蛛行 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 阳光敏感 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 蛛网行者 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/drider.json)
- [总索引与运行说明](../README.md)
