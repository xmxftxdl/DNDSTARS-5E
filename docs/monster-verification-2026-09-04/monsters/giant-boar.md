...
规则 ID：`srd-5.1:giant-boar`。结论：**本次已覆盖范围通过**。
...
通过：獠牙。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 308/320。

> 巨型野猪 使用獠牙攻击 验证目标：獠牙 → 验证目标 12+5=17 命中；共造成 12 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 獠牙 / `tusk` | headless | weapon-attack | 通过 | miss、hit、critical |
...
- [本怪物字段与结果证据](../evidence/giant-boar.json)
- [总索引与运行说明](../README.md)
