...
规则 ID：`srd-5.1:pseudodragon`。结论：**本次已覆盖范围通过**。
...
> 伪龙 使用啃咬攻击 验证目标：啃咬 → 验证目标 15+4=19 命中；共造成 6 点伤害。
...
| 动作 | 螫刺 / `sting` | headless | weapon-attack | 通过 | miss、hit、critical、sting-poisoned-unconscious:apply、sting-poisoned-unconscious:save、sting-poisoned-unconscious:condition-immune |
...
| --- | --- | --- | --- | --- |
| 敏锐感官 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 魔法抗性 | headless | magic-resistance | 通过 | magical、physical |
| 有限心灵感应 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/pseudodragon.json)
- [总索引与运行说明](../README.md)
