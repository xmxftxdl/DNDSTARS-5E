...
规则 ID：`srd-5.1:giant-octopus`。结论：**本次已覆盖范围通过**。
...
通过：触手群攻。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 巨型章鱼 使用触手群攻攻击 验证目标：触手群攻 → 验证目标 3+5=8 未命中；共造成 0 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 触手群攻 / `tentacles` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 墨汁云 / `ink-cloud` | headless | persistent-area | 通过 | minimum、maximum、no-resource、wrong-environment |
...
| 水下伪装 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 水下呼吸 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/giant-octopus.json)
- [总索引与运行说明](../README.md)
