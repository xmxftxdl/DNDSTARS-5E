...
规则 ID：`srd-5.1:zombie`。结论：**本次已覆盖范围通过**。
...
通过：猛击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 314/320。

> 僵尸 使用猛击攻击 验证目标：猛击 → 验证目标 12+3=15 命中；共造成 6 点伤害。
...
- [本怪物字段与结果证据](../evidence/zombie.json)
- [总索引与运行说明](../README.md)
