...
规则 ID：`srd-5.1:ice-mephit`。结论：**本次已覆盖范围通过**。
...
通过：双爪。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 317/320。

> 冰魔蝠 使用双爪攻击 验证目标：双爪 → 验证目标 17+3=20 命中；共造成 3 点伤害。
...
| 动作 | 霜冻吐息 / `frost-breath` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target、exhausted-resource |
...
- [本怪物字段与结果证据](../evidence/ice-mephit.json)
- [总索引与运行说明](../README.md)
