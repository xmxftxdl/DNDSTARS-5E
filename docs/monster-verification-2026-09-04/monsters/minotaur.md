...
规则 ID：`srd-5.1:minotaur`。结论：**本次已覆盖范围通过**。
...
通过：巨斧。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 313/320。

> 牛头人 使用巨斧攻击 验证目标：巨斧 → 验证目标 4+6=10 命中；共造成 7 点伤害。
...
| 迷宫记忆 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/minotaur.json)
- [总索引与运行说明](../README.md)
