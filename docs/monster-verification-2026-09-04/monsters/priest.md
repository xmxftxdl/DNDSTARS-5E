...
规则 ID：`srd-5.1:priest`。结论：**本次已覆盖范围通过**。
...
通过：硬头锤。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 318/320。

> 祭司 使用硬头锤攻击 验证目标：硬头锤 → 验证目标 10+2=12 命中；共造成 2 点伤害。
...
| --- | --- | --- | --- | --- |
| 神圣威能 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/priest.json)
- [总索引与运行说明](../README.md)
