...
规则 ID：`srd-5.1:ettercap`。结论：**本次已覆盖范围通过**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 308/320。

> 蜘蛛人 使用啃咬攻击 验证目标：啃咬 → 验证目标 17+4=21 命中；共造成 12 点伤害。
...
| 动作 | 啃咬 / `bite` | headless | weapon-attack | 通过 | miss、hit、critical、bite-poisoned:apply、bite-poisoned:save、bite-poisoned:condition-immune |
| 动作 | 双爪 / `claws` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 蛛网 / `web` | headless | weapon-attack | 通过 | miss、hit、critical、web-restraint:apply |
...
| 蛛网感知 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/ettercap.json)
- [总索引与运行说明](../README.md)
