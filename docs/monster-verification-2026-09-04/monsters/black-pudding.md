...
规则 ID：`srd-5.1:black-pudding`。结论：**Headless 已验证，仍有 DM 裁定边界**。
...
通过：伪足。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 300/320。

> 黑布丁 使用伪足攻击 验证目标：伪足 → 验证目标 17+5=22 命中；共造成 20 点伤害。
...
| 反应 | 分裂 / `split` | dm-adjudication | other | DM 裁定 / 非战斗 | — |
...
| --- | --- | --- | --- | --- |
| 无定形 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 腐蚀形态 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/black-pudding.json)
- [总索引与运行说明](../README.md)
