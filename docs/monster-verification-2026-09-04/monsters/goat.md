...
规则 ID：`srd-5.1:goat`。结论：**本次已覆盖范围通过**。
...
通过：冲撞。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 315/320。

> 山羊 使用冲撞攻击 验证目标：冲撞 → 验证目标 11+3=14 命中；共造成 5 点伤害。
