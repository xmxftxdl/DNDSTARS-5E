...
规则 ID：`srd-5.1:rakshasa`。结论：**本次已覆盖范围通过**。
...
通过：爪击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 311/320。

> 罗刹 使用爪击攻击 验证目标：爪击 → 验证目标 10+7=17 命中；共造成 9 点伤害。
...
| 动作 | 爪击 / `claw` | headless | weapon-attack | 通过 | miss、hit、critical、claw-rest-curse:apply |
...
| --- | --- | --- | --- | --- |
| 有限魔法免疫 | headless | limited-magic-immunity | 通过 | spell-level-6、spell-level-7、willing-ally |
...
- [本怪物字段与结果证据](../evidence/rakshasa.json)
- [总索引与运行说明](../README.md)
