...
规则 ID：`srd-5.1:merrow`。结论：**本次已覆盖范围通过**。
...
> 魔化人鱼 使用啃咬攻击 验证目标：啃咬 → 验证目标 3+6=9 未命中；共造成 0 点伤害。
...
| 动作 | 鱼叉 / `harpoon` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 多重攻击：啃咬 ×2 / `multiattack-bite-and-harpoon` | headless | multiattack | 通过 | complete-sequence |
...
- [本怪物字段与结果证据](../evidence/merrow.json)
- [总索引与运行说明](../README.md)
