...
规则 ID：`srd-5.1:knight`。结论：**Headless 已验证，仍有 DM 裁定边界**。
...
通过：巨剑。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 骑士 使用巨剑攻击 验证目标：巨剑 → 验证目标 4+5=9 未命中；共造成 0 点伤害。
...
| 动作 | 领导力 / `leadership` | dm-adjudication | other | DM 裁定 / 非战斗 | — |
| 反应 | 招架 / `parry` | headless | parry | 通过 | SRD monster Parry in Headless combat applies the exact +2 AC reaction for 'knight' |
...
- [本怪物字段与结果证据](../evidence/knight.json)
- [总索引与运行说明](../README.md)
