...
规则 ID：`srd-5.1:noble`。结论：**本次已覆盖范围通过**。
...
通过：刺剑。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 318/320。

> 贵族 使用刺剑攻击 验证目标：刺剑 → 验证目标 10+3=13 命中；共造成 2 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 刺剑 / `rapier` | headless | weapon-attack | 通过 | miss、hit、critical |
| 反应 | 招架 / `parry` | headless | parry | 通过 | SRD monster Parry in Headless combat applies the exact +2 AC reaction for 'noble' |
...
- [本怪物字段与结果证据](../evidence/noble.json)
- [总索引与运行说明](../README.md)
