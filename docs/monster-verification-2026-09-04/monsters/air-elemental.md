...
规则 ID：`srd-5.1:air-elemental`。结论：**Headless 已验证，仍有 DM 裁定边界**。

## 已知问题与边界

- 保留边界：旋风的首次力量豁免与伤害可结构化，但随机方向抛飞、撞击物体的逐 10 尺伤害、撞上另一生物时的第二次敏捷豁免与倒地必须绑定同一次移动事务；完整碰撞链建立前继续由 DM 裁定。

当前仍有 0 个 Headless 动作未被本次目录矩阵完整确认，1 个动作使用 DM 裁定或属于非战斗能力。

## 实际 UI

通过：猛击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 302/320。

> 气元素 使用猛击攻击 验证目标：猛击 → 验证目标 9+8=17 命中；共造成 18 点伤害。
...
| 动作 | 旋风 / `whirlwind` | dm-adjudication | other | DM 裁定 / 非战斗 | — |
...
| --- | --- | --- | --- | --- |
| 气体形态 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/air-elemental.json)
- [总索引与运行说明](../README.md)
