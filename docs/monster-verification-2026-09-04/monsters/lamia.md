...
规则 ID：`srd-5.1:lamia`。结论：**本次已覆盖范围通过**。
...
通过：双爪。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 拉弥亚 使用双爪攻击 验证目标：双爪 → 验证目标 1+5=6 未命中；共造成 0 点伤害。
...
| 动作 | 迷醉之触 / `intoxicating-touch` | headless | weapon-attack | 通过 | miss、hit、critical、intoxicating-touch-curse:apply |
| 动作 | 多重攻击：双爪 ×2 / `multiattack-claws-and-intoxicating-touch` | headless | multiattack | 通过 | complete-sequence |
...
- [本怪物字段与结果证据](../evidence/lamia.json)
- [总索引与运行说明](../README.md)
