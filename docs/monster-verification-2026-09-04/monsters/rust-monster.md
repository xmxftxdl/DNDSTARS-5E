...
规则 ID：`srd-5.1:rust-monster`。结论：**Headless 已验证，仍有 DM 裁定边界**。
...
> 锈蚀怪 使用啃咬攻击 验证目标：啃咬 → 验证目标 5+3=8 未命中；共造成 0 点伤害。
...
| 动作 | 触须 / `antennae` | dm-adjudication | other | DM 裁定 / 非战斗 | — |
...
| --- | --- | --- | --- | --- |
| 嗅铁 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 锈蚀金属 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/rust-monster.json)
- [总索引与运行说明](../README.md)
