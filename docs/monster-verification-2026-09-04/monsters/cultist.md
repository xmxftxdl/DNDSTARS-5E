...
规则 ID：`srd-5.1:cultist`。结论：**本次已覆盖范围通过**。
...
通过：弯刀。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 318/320。

> 邪教徒 使用弯刀攻击 验证目标：弯刀 → 验证目标 11+3=14 命中；共造成 2 点伤害。
...
- [本怪物字段与结果证据](../evidence/cultist.json)
- [总索引与运行说明](../README.md)
