...
规则 ID：`srd-5.1:wight`。结论：**本次已覆盖范围通过**。
...
通过：生命吸取。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 314/320。

> 尸鬼 使用生命吸取攻击 验证目标：生命吸取 → 验证目标 14+4=18 命中；共造成 6 点伤害。
...
| 动作 | 生命吸取 / `life-drain` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| 动作 | 多重攻击：长剑 ×2 / `multiattack-longsword` | headless | multiattack | 通过 | complete-sequence |
| 动作 | 多重攻击：生命吸取 ×2 / `multiattack-life-drain-and-longsword` | headless | multiattack | 通过 | complete-sequence |
...
- [本怪物字段与结果证据](../evidence/wight.json)
- [总索引与运行说明](../README.md)
