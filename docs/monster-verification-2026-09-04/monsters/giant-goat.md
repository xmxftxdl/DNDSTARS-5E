...
规则 ID：`srd-5.1:giant-goat`。结论：**本次已覆盖范围通过**。
...
通过：冲撞。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 310/320。

> 巨型山羊 使用冲撞攻击 验证目标：冲撞 → 验证目标 17+5=22 命中；共造成 10 点伤害。
...
| 稳步 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/giant-goat.json)
- [总索引与运行说明](../README.md)
