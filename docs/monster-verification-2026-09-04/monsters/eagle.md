...
规则 ID：`srd-5.1:eagle`。结论：**本次已覆盖范围通过**。
...
通过：利爪。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 316/320。

> 鹰 使用利爪攻击 验证目标：利爪 → 验证目标 7+4=11 命中；共造成 4 点伤害。
...
- [本怪物字段与结果证据](../evidence/eagle.json)
- [总索引与运行说明](../README.md)
