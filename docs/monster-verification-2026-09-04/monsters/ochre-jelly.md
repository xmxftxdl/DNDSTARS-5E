...
规则 ID：`srd-5.1:ochre-jelly`。结论：**Headless 已验证，仍有 DM 裁定边界**。
...
通过：伪足。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 305/320。

> 赭冻怪 使用伪足攻击 验证目标：伪足 → 验证目标 8+4=12 命中；共造成 15 点伤害。
...
- [本怪物字段与结果证据](../evidence/ochre-jelly.json)
- [总索引与运行说明](../README.md)
