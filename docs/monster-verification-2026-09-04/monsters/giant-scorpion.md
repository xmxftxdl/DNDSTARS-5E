...
规则 ID：`srd-5.1:giant-scorpion`。结论：**本次已覆盖范围通过**。
...
通过：爪击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 315/320。

> 巨蝎 使用爪击攻击 验证目标：爪击 → 验证目标 13+4=17 命中；共造成 5 点伤害。
...
| 动作 | 螫刺 / `sting` | headless | weapon-attack | 通过 | miss、hit、critical |
...
- [本怪物字段与结果证据](../evidence/giant-scorpion.json)
- [总索引与运行说明](../README.md)
