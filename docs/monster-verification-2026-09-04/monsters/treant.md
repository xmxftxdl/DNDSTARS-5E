...
规则 ID：`srd-5.1:treant`。结论：**Headless 已验证，仍有 DM 裁定边界**。

## 已知问题与边界

- 保留边界：活化树木要求选择真实地形树，创建带临时属性覆盖的树人实体，并跟踪一天、距离、来源死亡、奖励动作解除和重新扎根。当前召唤模型没有地形实体选择与属性覆盖。

当前仍有 0 个 Headless 动作未被本次目录矩阵完整确认，1 个动作使用 DM 裁定或属于非战斗能力。

## 实际 UI

通过：猛击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 300/320。

> 树人 使用猛击攻击 验证目标：猛击 → 验证目标 8+10=18 命中；共造成 20 点伤害。
...
| 动作 | 活化树木 / `animate-trees` | dm-adjudication | other | DM 裁定 / 非战斗 | — |
...
- [本怪物字段与结果证据](../evidence/treant.json)
- [总索引与运行说明](../README.md)
