...
规则 ID：`srd-5.1:guard`。结论：**本次已覆盖范围通过**。
...
- 通过：7073/7074 双端真实页面执行长矛 (Two-Handed Melee)。目标 HP 320 → 318，动作余量变为 0，DM 与玩家日志一致。

> 守卫 使用长矛 (Two-Handed Melee)攻击 验证目标：长矛 (Two-Handed Melee) → 验证目标 10+3=13 命中；共造成 2 点伤害。
...
- [本怪物字段与结果证据](../evidence/guard.json)
- [战斗动作真实 UI 证据](../evidence/guard-combat-ui.json)
- [总索引与运行说明](../README.md)
