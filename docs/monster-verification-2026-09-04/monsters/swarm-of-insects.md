# 昆虫群（Swarm of Insects）

规则 ID：`srd-5.1:swarm-of-insects`。结论：**本次已覆盖范围通过**。
...
通过：群体啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 308/320。

> 昆虫群 使用群体啃咬攻击 验证目标：群体啃咬 → 验证目标 14+3=17 命中；共造成 12 点伤害。
...
- [本怪物字段与结果证据](../evidence/swarm-of-insects.json)
- [总索引与运行说明](../README.md)
