...
规则 ID：`srd-5.1:grick`。结论：**本次已覆盖范围通过**。
...
通过：触手群攻。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 314/320。

> 格里克异怪 使用触手群攻攻击 验证目标：触手群攻 → 验证目标 19+4=23 命中；共造成 6 点伤害。
...
- [本怪物字段与结果证据](../evidence/grick.json)
- [总索引与运行说明](../README.md)
