...
规则 ID：`srd-5.1:chuul`。结论：**本次已覆盖范围通过**。
...
通过：螯击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 303/320。

> 楚尔异怪 使用螯击攻击 验证目标：螯击 → 验证目标 20+6=26 命中；共造成 17 点伤害。
...
| 动作 | 螯击 / `pincer` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 触手群攻 / `tentacles` | headless | saving-throw-condition | 通过 | minimum、maximum、no-resource、out-of-range |
| 动作 | 多重攻击：螯击 ×3 / `multiattack-pincers-and-tentacles` | headless | multiattack | 通过 | complete-sequence |
...
| 感知魔法 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/chuul.json)
- [总索引与运行说明](../README.md)
