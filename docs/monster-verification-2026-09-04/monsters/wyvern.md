...
规则 ID：`srd-5.1:wyvern`。结论：**本次已覆盖范围通过**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 310/320。

> 双足飞龙 使用啃咬攻击 验证目标：啃咬 → 验证目标 18+7=25 命中；共造成 10 点伤害。
...
| 动作 | 尾针 / `stinger` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 多重攻击：双爪 ×2 / `multiattack-claws-and-stinger` | headless | multiattack | 通过 | complete-sequence |
| 动作 | 多重攻击：啃咬 ×2 / `multiattack-bite-and-claws` | headless | multiattack | 通过 | complete-sequence |
...
- [本怪物字段与结果证据](../evidence/wyvern.json)
- [总索引与运行说明](../README.md)
