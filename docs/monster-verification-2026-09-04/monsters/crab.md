...
规则 ID：`srd-5.1:crab`。结论：**本次已覆盖范围通过**。
...
通过：爪击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 319/320。

> 螃蟹 使用爪击攻击 验证目标：爪击 → 验证目标 20+0=20 命中；共造成 1 点伤害。
