...
规则 ID：`srd-5.1:adult-black-dragon`。结论：**本次已覆盖范围通过**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 275/320。

> 成年黑龙 使用啃咬攻击 验证目标：啃咬 → 验证目标 20+11=31 命中；共造成 45 点伤害。
...
- [本怪物字段与结果证据](../evidence/adult-black-dragon.json)
- [总索引与运行说明](../README.md)
