# 熊人（混合形态）（Werebear, Hybrid Form）

规则 ID：`srd-5.1:werebear-hybrid`。结论：**本次已覆盖范围通过**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 303/320。

> 熊人（混合形态） 使用啃咬攻击 验证目标：啃咬 → 验证目标 13+7=20 命中；共造成 17 点伤害。
...
| 动作 | 多重攻击：巨斧 ×2 / `multiattack-greataxe` | headless | multiattack | 通过 | complete-sequence |
...
- [本怪物字段与结果证据](../evidence/werebear-hybrid.json)
- [总索引与运行说明](../README.md)
