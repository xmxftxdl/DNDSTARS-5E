...
规则 ID：`srd-5.1:rhinoceros`。结论：**本次已覆盖范围通过**。
...
通过：顶撞。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 300/320。

> 犀牛 使用顶撞攻击 验证目标：顶撞 → 验证目标 9+7=16 命中；共造成 20 点伤害。
...
- [本怪物字段与结果证据](../evidence/rhinoceros.json)
- [总索引与运行说明](../README.md)
