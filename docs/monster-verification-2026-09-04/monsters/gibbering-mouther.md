# 喋喋不休怪（Gibbering Mouther）

规则 ID：`srd-5.1:gibbering-mouther`。结论：**本次已覆盖范围通过**。
...
通过：群体啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 喋喋不休怪 使用群体啃咬攻击 验证目标：群体啃咬 → 验证目标 3+2=5 未命中；共造成 0 点伤害。
...
| 动作 | 群体啃咬 / `bites` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 致盲唾液 / `blinding-spittle` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target、exhausted-resource |
| 动作 | 多重攻击: 群体啃咬 / `multiattack-bites-only` | headless | multiattack | 通过 | complete-sequence |
...
| --- | --- | --- | --- | --- |
| 异变地面 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 胡言乱语 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/gibbering-mouther.json)
- [总索引与运行说明](../README.md)
