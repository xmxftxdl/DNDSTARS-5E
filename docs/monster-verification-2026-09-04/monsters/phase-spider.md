...
规则 ID：`srd-5.1:phase-spider`。结论：**本次已覆盖范围通过**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 296/320。

> 相位蜘蛛 使用啃咬攻击 验证目标：啃咬 → 验证目标 15+4=19 命中；共造成 24 点伤害。
...
| --- | --- | --- | --- | --- |
| 以太跃迁 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/phase-spider.json)
- [总索引与运行说明](../README.md)
