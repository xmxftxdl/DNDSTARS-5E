...
规则 ID：`srd-5.1:sahuagin`。结论：**本次已覆盖范围通过**。
...
> 沙华鱼人 使用啃咬攻击 验证目标：啃咬 → 验证目标 1+3=4 未命中；共造成 0 点伤害。
...
| 动作 | 多重攻击：啃咬 ×2 / `multiattack-bite-and-spear` | headless | multiattack | 通过 | complete-sequence |
...
| --- | --- | --- | --- | --- |
| 嗜血狂暴 | headless | blood-frenzy | 通过 | full-target-hp、wounded-target |
| 有限两栖 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 鲨鱼心灵感应 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/sahuagin.json)
- [总索引与运行说明](../README.md)
