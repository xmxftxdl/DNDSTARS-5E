...
规则 ID：`srd-5.1:imp`。结论：**本次已覆盖范围通过**。
...
通过：螫刺（野兽形态为啃咬）。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 310/320。

> 小魔鬼 使用螫刺（野兽形态为啃咬）攻击 验证目标：螫刺（野兽形态为啃咬） → 验证目标 11+5=16 命中；共造成 10 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 螫刺（野兽形态为啃咬） / `sting-bite-in-beast-form` | headless | weapon-attack | 通过 | miss、hit、critical |
