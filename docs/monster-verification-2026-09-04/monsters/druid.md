...
规则 ID：`srd-5.1:druid`。结论：**本次已覆盖范围通过**。
...
通过：长棍。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 315/320。

> 德鲁伊 使用长棍攻击 验证目标：长棍 → 验证目标 15+2=17 命中；共造成 5 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 长棍 / `quarterstaff` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 长棍 (Two-Handed) / `quarterstaff-two-handed` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 长棍 (Shillelagh) / `quarterstaff-shillelagh` | headless | weapon-attack | 通过 | miss、hit、critical |
...
- [本怪物字段与结果证据](../evidence/druid.json)
- [总索引与运行说明](../README.md)
