...
规则 ID：`srd-5.1:nalfeshnee`。结论：**本次已覆盖范围通过**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 293/320。

> 纳尔弗魔 使用啃咬攻击 验证目标：啃咬 → 验证目标 7+10=17 命中；共造成 27 点伤害。
...
| 动作 | 恐怖光晕 / `horror-nimbus` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target、exhausted-resource |
| 动作 | 传送 / `teleport` | headless | teleport | 通过 | minimum、maximum、no-resource |
| 动作 | 多重攻击: 啃咬 / `multiattack-weapons-only` | headless | multiattack | 通过 | complete-sequence |
...
- [本怪物字段与结果证据](../evidence/nalfeshnee.json)
- [总索引与运行说明](../README.md)
