...
规则 ID：`srd-5.1:mummy`。结论：**本次已覆盖范围通过**。
...
通过：腐烂拳击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 木乃伊 使用腐烂拳击攻击 验证目标：腐烂拳击 → 验证目标 2+5=7 未命中；共造成 0 点伤害。
...
- [本怪物字段与结果证据](../evidence/mummy.json)
- [总索引与运行说明](../README.md)
