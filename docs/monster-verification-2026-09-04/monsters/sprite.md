...
规则 ID：`srd-5.1:sprite`。结论：**Headless 已验证，仍有 DM 裁定边界**。
...
通过：长剑。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 319/320。

> 小妖精 使用长剑攻击 验证目标：长剑 → 验证目标 10+2=12 命中；共造成 1 点伤害。
...
| 动作 | 短弓 / `shortbow` | headless | weapon-attack | 通过 | miss、hit、critical、shortbow-poisoned-unconscious:apply、shortbow-poisoned-unconscious:save、shortbow-poisoned-unconscious:condition-immune |
| 动作 | 洞察心灵 / `heart-sight` | dm-adjudication | other | DM 裁定 / 非战斗 | — |
...
- [本怪物字段与结果证据](../evidence/sprite.json)
- [总索引与运行说明](../README.md)
