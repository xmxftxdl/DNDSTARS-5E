...
规则 ID：`srd-5.1:triceratops`。结论：**本次已覆盖范围通过**。
...
通过：顶撞。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 298/320。

> 三角龙 使用顶撞攻击 验证目标：顶撞 → 验证目标 17+9=26 命中；共造成 22 点伤害。
...
| 动作 | 践踏 / `stomp` | headless | weapon-attack | 通过 | miss、hit、critical |
| 附赠动作 | 践踏冲锋：践踏 / `trampling-stomp-bonus-action` | headless | weapon-attack | 通过 | miss、hit、critical |
...
- [本怪物字段与结果证据](../evidence/triceratops.json)
- [总索引与运行说明](../README.md)
