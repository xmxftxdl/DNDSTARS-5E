# 吸血鬼（本体形态）（Vampire, Vampire Form）

规则 ID：`srd-5.1:vampire-vampire`。结论：**Headless 已验证，仍有 DM 裁定边界**。
...
通过：徒手打击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 315/320。

> 吸血鬼（本体形态） 使用徒手打击攻击 验证目标：徒手打击 → 验证目标 5+9=14 命中；共造成 5 点伤害。

### 传奇与特殊动作专项

- 通过：移动（场景 3）；DM 日志：吸血鬼（本体形态）使用传奇动作移动。
...
| 动作 | 徒手打击 / `unarmed-strike` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| 动作 | 徒手打击 (Grapple) / `unarmed-strike-grapple` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 多重攻击：徒手打击 ×2 / `multiattack-unarmed-strike-and-bite` | headless | multiattack | 通过 | complete-sequence |
| 动作 | 多重攻击：徒手打击 ×2 / `multiattack-unarmed-strike-and-grapple` | headless | multiattack | 通过 | complete-sequence |
| 动作 | 多重攻击：unarmed-strike-grapple ×2 / `multiattack-grapple-and-unarmed-strike` | headless | multiattack | 通过 | complete-sequence |
| 动作 | 多重攻击：unarmed-strike-grapple ×2 / `multiattack-unarmed-grapples` | headless | multiattack | 通过 | complete-sequence |
| 动作 | 多重攻击：unarmed-strike-grapple ×2 / `multiattack-unarmed-grapple-and-bite` | headless | multiattack | 通过 | complete-sequence |
| 传奇动作 | 移动 / `move` | headless | grant-movement | 通过 | minimum、maximum、no-resource、no-points、move、move-too-far、window-expired |
| 传奇动作 | 徒手打击 / `legendary-unarmed-strike` | headless | other | 通过 | miss、hit、critical |
...
- [本怪物字段与结果证据](../evidence/vampire-vampire.json)
- [总索引与运行说明](../README.md)
