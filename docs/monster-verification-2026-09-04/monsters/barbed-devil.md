...
规则 ID：`srd-5.1:barbed-devil`。结论：**本次已覆盖范围通过**。
...
通过：爪击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 314/320。

> 针刺魔 使用爪击攻击 验证目标：爪击 → 验证目标 12+6=18 命中；共造成 6 点伤害。
...
| --- | --- | --- | --- | --- |
| 倒刺外皮 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/barbed-devil.json)
- [总索引与运行说明](../README.md)
