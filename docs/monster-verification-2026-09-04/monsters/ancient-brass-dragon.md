...
规则 ID：`srd-5.1:ancient-brass-dragon`。结论：**Headless 已验证，仍有 DM 裁定边界**。
...
通过：啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 302/320。

> 远古黄铜龙 使用啃咬攻击 验证目标：啃咬 → 验证目标 16+14=30 命中；共造成 18 点伤害。
...
- [本怪物字段与结果证据](../evidence/ancient-brass-dragon.json)
- [总索引与运行说明](../README.md)
