...
规则 ID：`srd-5.1:ogre`。结论：**本次已覆盖范围通过**。
...
通过：巨棒。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 307/320。

> 食人魔 使用巨棒攻击 验证目标：巨棒 → 验证目标 19+6=25 命中；共造成 13 点伤害。
