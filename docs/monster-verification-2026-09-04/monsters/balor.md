...
规则 ID：`srd-5.1:balor`。结论：**本次已覆盖范围通过**。
...
通过：长剑。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 280/320。

> 巴洛炎魔 使用长剑攻击 验证目标：长剑 → 验证目标 4+14=18 命中；共造成 40 点伤害。
...
| 动作 | 长鞭 / `whip` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| --- | --- | --- | --- | --- |
| 临死爆发 | headless | death-area-saving-throw | 通过 | failed-save、successful-save、outside-radius、replay-rejected |
| 火焰灵光 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/balor.json)
- [总索引与运行说明](../README.md)
