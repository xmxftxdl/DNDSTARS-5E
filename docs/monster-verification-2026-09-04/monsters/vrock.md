...
规则 ID：`srd-5.1:vrock`。结论：**本次已覆盖范围通过**。
...
通过：喙击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 310/320。

> 弗洛魔 使用喙击攻击 验证目标：喙击 → 验证目标 4+6=10 命中；共造成 10 点伤害。
...
| 动作 | 喙击 / `beak` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 利爪 / `talons` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 孢子 / `spores` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target、exhausted-resource |
| 动作 | 震慑尖啸 / `stunning-screech` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target、exhausted-resource |
...
- [本怪物字段与结果证据](../evidence/vrock.json)
- [总索引与运行说明](../README.md)
