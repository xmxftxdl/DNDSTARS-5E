...
规则 ID：`srd-5.1:cult-fanatic`。结论：**本次已覆盖范围通过**。
...
通过：匕首。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 317/320。

> 邪教狂信徒 使用匕首攻击 验证目标：匕首 → 验证目标 16+4=20 命中；共造成 3 点伤害。
...
| --- | --- | --- | --- | --- |
| 黑暗奉献 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/cult-fanatic.json)
- [总索引与运行说明](../README.md)
