...
规则 ID：`srd-5.1:marilith`。结论：**本次已覆盖范围通过**。
...
通过：长剑。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 303/320。

> 六臂蛇魔 使用长剑攻击 验证目标：长剑 → 验证目标 7+9=16 命中；共造成 17 点伤害。
...
| 动作 | 传送 / `teleport` | headless | teleport | 通过 | minimum、maximum、no-resource |
| 反应 | 招架 / `parry` | headless | parry | 通过 | SRD monster Parry in Headless combat applies the exact +5 AC reaction for 'marilith' |
...
| 多重反应 | headless | reactive | 通过 | SRD monster Parry in Headless combat lets Marilith use Reactive once on each different creature turn、SRD monster Parry in Headless combat shares one Reactive budget between an opportunity attack and Parry on the same turn |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/marilith.json)
- [总索引与运行说明](../README.md)
