...
规则 ID：`srd-5.1:spy`。结论：**本次已覆盖范围通过**。
...
通过：短剑。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 317/320。

> 间谍 使用短剑攻击 验证目标：短剑 → 验证目标 17+4=21 命中；共造成 3 点伤害。
...
| --- | --- | --- | --- | --- |
| 狡诈动作 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 偷袭（每回合 1 次） | headless | sneak-attack | 通过 | SRD precision attack traits in the Headless runtime 'Spy Sneak Attack' waits for the first actual hit and cannot be reused in the same turn |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/spy.json)
- [总索引与运行说明](../README.md)
