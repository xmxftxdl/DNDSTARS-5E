...
规则 ID：`srd-5.1:boar`。结论：**本次已覆盖范围通过**。
...
通过：獠牙。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 314/320。

> 野猪 使用獠牙攻击 验证目标：獠牙 → 验证目标 10+3=13 命中；共造成 6 点伤害。
