...
规则 ID：`srd-5.1:wereboar-human`。结论：**本次已覆盖范围通过**。
...
通过：巨锤。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 313/320。

> 野猪人（人类形态） 使用巨锤攻击 验证目标：巨锤 → 验证目标 19+5=24 命中；共造成 7 点伤害。
...
- [本怪物字段与结果证据](../evidence/wereboar-human.json)
- [总索引与运行说明](../README.md)
