...
规则 ID：`srd-5.1:specter`。结论：**本次已覆盖范围通过**。
...
通过：生命吸取。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 幽魂 使用生命吸取攻击 验证目标：生命吸取 → 验证目标 2+4=6 未命中；共造成 0 点伤害。
...
- [本怪物字段与结果证据](../evidence/specter.json)
- [总索引与运行说明](../README.md)
