...
规则 ID：`srd-5.1:roc`。结论：**本次已覆盖范围通过**。
...
通过：喙击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 294/320。

> 鹏鸟 使用喙击攻击 验证目标：喙击 → 验证目标 9+13=22 命中；共造成 26 点伤害。
