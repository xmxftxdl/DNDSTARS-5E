# 野猪人（混合形态）（Wereboar, Hybrid Form）

规则 ID：`srd-5.1:wereboar-hybrid`。结论：**本次已覆盖范围通过**。
...
通过：巨锤。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 311/320。

> 野猪人（混合形态） 使用巨锤攻击 验证目标：巨锤 → 验证目标 9+5=14 命中；共造成 9 点伤害。
...
| 动作 | 巨锤 / `maul` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 双獠牙 / `tusks` | headless | weapon-attack | 通过 | miss、hit、critical、tusks-lycanthropy:apply、tusks-lycanthropy:save、tusks-lycanthropy:excluded-type |
| 动作 | 多重攻击：巨锤 ×2 / `multiattack-maul-and-tusks` | headless | multiattack | 通过 | complete-sequence |
...
| 冲锋（仅野猪或混合形态） | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 坚韧不屈 | headless | relentless | 通过 | at-threshold、above-threshold、already-zero |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/wereboar-hybrid.json)
- [总索引与运行说明](../README.md)
