...
规则 ID：`srd-5.1:stone-giant`。结论：**Headless 已验证，仍有 DM 裁定边界**。

## 已知问题与边界

- 保留边界：接岩石只对“岩石或类似物体”的投掷命中触发，随后还要消耗反应并通过 DC 10 敏捷豁免才能完全免除钝击伤害。当前通用攻击载荷没有可信的投射物类别字段，不能把所有远程攻击误判成岩石。

当前仍有 0 个 Headless 动作未被本次目录矩阵完整确认，1 个动作使用 DM 裁定或属于非战斗能力。

## 实际 UI

通过：巨棒。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 298/320。

> 石巨人 使用巨棒攻击 验证目标：巨棒 → 验证目标 14+9=23 命中；共造成 22 点伤害。
...
| 动作 | 巨棒 / `greatclub` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 投石 / `rock` | headless | weapon-attack | 通过 | miss、hit、critical |
| 反应 | 接石 / `rock-catching` | dm-adjudication | other | DM 裁定 / 非战斗 | — |
...
- [本怪物字段与结果证据](../evidence/stone-giant.json)
- [总索引与运行说明](../README.md)
