...
规则 ID：`srd-5.1:commoner`。结论：**本次已覆盖范围通过**。
...
通过：木棒。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 319/320。

> 平民 使用木棒攻击 验证目标：木棒 → 验证目标 18+2=20 命中；共造成 1 点伤害。
...
- [本怪物字段与结果证据](../evidence/commoner.json)
- [总索引与运行说明](../README.md)
