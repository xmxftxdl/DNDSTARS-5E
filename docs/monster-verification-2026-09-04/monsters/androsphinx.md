...
规则 ID：`srd-5.1:androsphinx`。结论：**本次已覆盖范围通过**。
...
通过：爪击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 306/320。

> 雄性斯芬克斯 使用爪击攻击 验证目标：爪击 → 验证目标 19+12=31 命中；共造成 14 点伤害。
...
| 动作 | 咆哮 / `roar` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target、exhausted-resource |
...
- [本怪物字段与结果证据](../evidence/androsphinx.json)
- [总索引与运行说明](../README.md)
