...
规则 ID：`srd-5.1:giant-wasp`。结论：**本次已覆盖范围通过**。
...
通过：螫刺。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 308/320。

> 巨蜂 使用螫刺攻击 验证目标：螫刺 → 验证目标 18+4=22 命中；共造成 12 点伤害。
...
- [本怪物字段与结果证据](../evidence/giant-wasp.json)
- [总索引与运行说明](../README.md)
