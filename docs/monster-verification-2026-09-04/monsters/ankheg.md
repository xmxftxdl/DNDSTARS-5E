...
规则 ID：`srd-5.1:ankheg`。结论：**本次已覆盖范围通过**。
...
> 安赫格掘穴虫 使用啃咬攻击 验证目标：啃咬 → 验证目标 12+5=17 命中；共造成 17 点伤害。
...
| 动作 | 强酸喷射 / `acid-spray` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target、exhausted-resource |
...
- [本怪物字段与结果证据](../evidence/ankheg.json)
- [总索引与运行说明](../README.md)
