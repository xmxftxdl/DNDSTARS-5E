...
规则 ID：`srd-5.1:kobold`。结论：**本次已覆盖范围通过**。
...
通过：匕首。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 狗头人 使用匕首攻击 验证目标：匕首 → 验证目标 5+4=9 未命中；共造成 0 点伤害。
...
| 动作 | 投石索 / `sling` | headless | weapon-attack | 通过 | miss、hit、critical |
...
- [本怪物字段与结果证据](../evidence/kobold.json)
- [总索引与运行说明](../README.md)
