...
规则 ID：`srd-5.1:azer`。结论：**本次已覆盖范围通过**。
...
通过：战锤。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 309/320。

> 艾泽 使用战锤攻击 验证目标：战锤 → 验证目标 18+5=23 命中；共造成 11 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 战锤 / `warhammer` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 战锤 (Two-Handed) / `warhammer-two-handed` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| --- | --- | --- | --- | --- |
| 灼热躯体 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 灼热武器 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
