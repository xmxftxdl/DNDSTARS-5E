...
规则 ID：`srd-5.1:gray-ooze`。结论：**本次已覆盖范围通过**。
...
通过：伪足。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 灰泥怪 使用伪足攻击 验证目标：伪足 → 验证目标 4+3=7 未命中；共造成 0 点伤害。
...
| 腐蚀金属 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/gray-ooze.json)
- [总索引与运行说明](../README.md)
