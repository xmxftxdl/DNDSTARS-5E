...
规则 ID：`srd-5.1:violet-fungus`。结论：**本次已覆盖范围通过**。
...
通过：腐烂之触。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 紫罗兰蕈 使用腐烂之触攻击 验证目标：腐烂之触 → 验证目标 5+2=7 未命中；共造成 0 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 多重攻击 / `multiattack` | headless | multiattack | 通过 | repeat-1、repeat-4 |
| 动作 | 腐烂之触 / `rotting-touch` | headless | weapon-attack | 通过 | miss、hit、critical |
...
- [本怪物字段与结果证据](../evidence/violet-fungus.json)
- [总索引与运行说明](../README.md)
