...
规则 ID：`srd-5.1:sea-hag`。结论：**Headless 已验证，仍有 DM 裁定边界**。
...
通过：双爪。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 311/320。

> 海鬼婆 使用双爪攻击 验证目标：双爪 → 验证目标 12+5=17 命中；共造成 9 点伤害。
...
| 动作 | 死亡瞪视 / `death-glare` | headless | saving-throw-terminal-effect | 通过 | minimum、maximum、no-resource、out-of-range、source-blinded、target-blinded、missing-condition |
| 动作 | 幻象外貌 / `illusory-appearance` | dm-adjudication | other | DM 裁定 / 非战斗 | — |
...
| 骇人外貌 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/sea-hag.json)
- [总索引与运行说明](../README.md)
