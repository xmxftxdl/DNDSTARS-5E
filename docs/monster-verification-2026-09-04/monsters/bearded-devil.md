...
规则 ID：`srd-5.1:bearded-devil`。结论：**本次已覆盖范围通过**。
...
通过：胡须。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 313/320。

> 须魔 使用胡须攻击 验证目标：胡须 → 验证目标 11+5=16 命中；共造成 7 点伤害。
...
| 动作 | 胡须 / `beard` | headless | weapon-attack | 通过 | miss、hit、critical、beard-poisoned:apply、beard-poisoned:save、beard-poisoned:condition-immune |
| 动作 | 长柄刀 / `glaive` | headless | weapon-attack | 通过 | miss、hit、critical、glaive-infernal-wound:apply、glaive-infernal-wound:save、glaive-infernal-wound:excluded-type |
...
| 坚定 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/bearded-devil.json)
- [总索引与运行说明](../README.md)
