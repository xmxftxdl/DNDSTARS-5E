...
规则 ID：`srd-5.1:drow`。结论：**本次已覆盖范围通过**。
...
通过：短剑。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 315/320。

> 卓尔 使用短剑攻击 验证目标：短剑 → 验证目标 18+4=22 命中；共造成 5 点伤害。
...
| 动作 | 手弩 / `hand-crossbow` | headless | weapon-attack | 通过 | miss、hit、critical、crossbow-poisoned-unconscious:apply、crossbow-poisoned-unconscious:save、crossbow-poisoned-unconscious:condition-immune |
