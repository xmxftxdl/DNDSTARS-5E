...
规则 ID：`srd-5.1:xorn`。结论：**本次已覆盖范围通过**。
...
> 索尔石怪 使用啃咬攻击 验证目标：啃咬 → 验证目标 1+6=7 未命中；共造成 0 点伤害。
...
| --- | --- | --- | --- | --- |
| 土遁 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 岩石伪装 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 宝藏感知 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/xorn.json)
- [总索引与运行说明](../README.md)
