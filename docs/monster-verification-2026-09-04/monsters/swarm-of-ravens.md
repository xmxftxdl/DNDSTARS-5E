...
规则 ID：`srd-5.1:swarm-of-ravens`。结论：**本次已覆盖范围通过**。
...
通过：群喙啄击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 313/320。

> 渡鸦群 使用群喙啄击攻击 验证目标：群喙啄击 → 验证目标 6+4=10 命中；共造成 7 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 群喙啄击 / `beaks` | headless | weapon-attack | 通过 | miss、hit、critical、half-hp |
...
- [本怪物字段与结果证据](../evidence/swarm-of-ravens.json)
- [总索引与运行说明](../README.md)
