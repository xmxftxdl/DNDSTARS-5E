...
规则 ID：`srd-5.1:stone-golem`。结论：**本次已覆盖范围通过**。
...
通过：猛击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 298/320。

> 石魔像 使用猛击攻击 验证目标：猛击 → 验证目标 8+10=18 命中；共造成 22 点伤害。
...
| 动作 | 缓慢 / `slow` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target、exhausted-resource |
...
- [本怪物字段与结果证据](../evidence/stone-golem.json)
- [总索引与运行说明](../README.md)
