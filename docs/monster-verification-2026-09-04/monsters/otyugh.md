...
规则 ID：`srd-5.1:otyugh`。结论：**本次已覆盖范围通过**。
...
> 奥图克 使用啃咬攻击 验证目标：啃咬 → 验证目标 1+6=7 未命中；共造成 0 点伤害。
...
| 动作 | 啃咬 / `bite` | headless | weapon-attack | 通过 | miss、hit、critical、bite-disease:apply、bite-disease:save |
| 动作 | 触手 / `tentacle` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 触手猛击 / `tentacle-slam` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target |
...
- [本怪物字段与结果证据](../evidence/otyugh.json)
- [总索引与运行说明](../README.md)
