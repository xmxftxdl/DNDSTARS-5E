# 胶质立方怪（Gelatinous Cube）

规则 ID：`srd-5.1:gelatinous-cube`。结论：**Headless 已验证，仍有 DM 裁定边界**。
...
通过：伪足。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 310/320。

> 胶质立方怪 使用伪足攻击 验证目标：伪足 → 验证目标 11+4=15 命中；共造成 10 点伤害。
...
| 动作 | 吞没 / `engulf` | dm-adjudication | other | DM 裁定 / 非战斗 | — |
...
| --- | --- | --- | --- | --- |
| 泥怪立方 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 透明 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/gelatinous-cube.json)
- [总索引与运行说明](../README.md)
