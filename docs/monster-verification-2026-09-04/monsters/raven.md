...
规则 ID：`srd-5.1:raven`。结论：**本次已覆盖范围通过**。
...
通过：喙击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 319/320。

> 渡鸦 使用喙击攻击 验证目标：喙击 → 验证目标 13+4=17 命中；共造成 1 点伤害。
...
- [本怪物字段与结果证据](../evidence/raven.json)
- [总索引与运行说明](../README.md)
