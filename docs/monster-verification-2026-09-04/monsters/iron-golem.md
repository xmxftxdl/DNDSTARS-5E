...
规则 ID：`srd-5.1:iron-golem`。结论：**本次已覆盖范围通过**。
...
通过：猛击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 铁魔像 使用猛击攻击 验证目标：猛击 → 验证目标 1+13=14 未命中；共造成 0 点伤害。
...
| 动作 | 剑 / `sword` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 毒气吐息 / `poison-breath` | headless | area-saving-throw | 通过 | failed-save、successful-save、duplicate-target、exhausted-resource |
| 动作 | 多重攻击：猛击 ×2 / `multiattack-slams` | headless | multiattack | 通过 | complete-sequence |
| 动作 | 多重攻击：剑 ×2 / `multiattack-sword-and-slam` | headless | multiattack | 通过 | complete-sequence |
...
| --- | --- | --- | --- | --- |
| 火焰吸收 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
| 不变形态 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/iron-golem.json)
- [总索引与运行说明](../README.md)
