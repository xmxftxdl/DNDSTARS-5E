...
规则 ID：`srd-5.1:oni`。结论：**Headless 已验证，仍有 DM 裁定边界**。
...
通过：爪击（仅鬼人形态）。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 314/320。

> 鬼人 使用爪击（仅鬼人形态）攻击 验证目标：爪击（仅鬼人形态） → 验证目标 8+7=15 命中；共造成 6 点伤害。
...
| 动作 | 爪击（仅鬼人形态） / `claw-oni-form-only` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 长柄刀 / `glaive` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 改变形态 / `change-shape` | dm-adjudication | other | DM 裁定 / 非战斗 | — |
| 动作 | 长柄刀（小型／中型形态） / `glaive-small-or-medium` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 多重攻击：长柄刀 ×2 / `multiattack-glaive` | headless | multiattack | 通过 | complete-sequence |
