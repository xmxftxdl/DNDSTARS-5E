...
规则 ID：`srd-5.1:mule`。结论：**本次已覆盖范围通过**。
...
通过：蹄击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 315/320。

> 骡子 使用蹄击攻击 验证目标：蹄击 → 验证目标 10+2=12 命中；共造成 5 点伤害。
...
| --- | --- | --- | --- | --- |
| 驮兽 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
