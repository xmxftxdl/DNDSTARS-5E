...
规则 ID：`srd-5.1:giant-vulture`。结论：**本次已覆盖范围通过**。
...
通过：喙击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 312/320。

> 巨型秃鹫 使用喙击攻击 验证目标：喙击 → 验证目标 16+4=20 命中；共造成 8 点伤害。
...
| --- | --- | --- | --- | --- |
| 敏锐视觉与嗅觉 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/giant-vulture.json)
- [总索引与运行说明](../README.md)
