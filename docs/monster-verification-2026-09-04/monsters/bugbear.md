...
规则 ID：`srd-5.1:bugbear`。结论：**本次已覆盖范围通过**。
...
通过：晨星锤。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 熊地精 使用晨星锤攻击 验证目标：晨星锤 → 验证目标 4+4=8 未命中；共造成 0 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 晨星锤 / `morningstar` | headless | weapon-attack | 通过 | miss、hit、critical |
...
| 突袭攻击 | headless | surprise-attack | 通过 | Bugbear and Doppelganger Surprise Attack settlement inherits bugbear primary damage and doubles all Surprise Attack dice on a critical |

特性列表表示目录现状；除上面明确列出的定向回归外，不以本次武器攻击代替回合开始、受击、死亡、重生等特性的独立触发验证。

## 复查材料

- [本怪物字段与结果证据](../evidence/bugbear.json)
- [总索引与运行说明](../README.md)
