...
规则 ID：`srd-5.1:dragon-turtle`。结论：**本次已覆盖范围通过**。
...
> 龙龟 使用啃咬攻击 验证目标：啃咬 → 验证目标 1+13=14 未命中；共造成 0 点伤害。
...
| 动作 | 多重攻击：啃咬 ×2 / `multiattack-bite-and-tail` | headless | multiattack | 通过 | complete-sequence |
...
- [本怪物字段与结果证据](../evidence/dragon-turtle.json)
- [总索引与运行说明](../README.md)
