...
规则 ID：`srd-5.1:warhorse-skeleton`。结论：**本次已覆盖范围通过**。
...
通过：蹄击。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 312/320。

> 战马骷髅 使用蹄击攻击 验证目标：蹄击 → 验证目标 7+6=13 命中；共造成 8 点伤害。
...
- [本怪物字段与结果证据](../evidence/warhorse-skeleton.json)
- [总索引与运行说明](../README.md)
