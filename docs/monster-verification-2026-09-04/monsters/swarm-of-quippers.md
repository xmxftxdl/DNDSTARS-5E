...
规则 ID：`srd-5.1:swarm-of-quippers`。结论：**本次已覆盖范围通过**。
...
通过：群体啃咬。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 304/320。

> 食人鱼群 使用群体啃咬攻击 验证目标：群体啃咬 → 验证目标 13+5=18 命中；共造成 16 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 群体啃咬 / `bites` | headless | weapon-attack | 通过 | miss、hit、critical、half-hp |
...
| 群集 | headless | swarm | 通过 | healing、temporary-hit-points |
...
- [本怪物字段与结果证据](../evidence/swarm-of-quippers.json)
- [总索引与运行说明](../README.md)
