...
规则 ID：`srd-5.1:efreeti`。结论：**本次已覆盖范围通过**。
...
通过：弯刀。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 火巨灵 使用弯刀攻击 验证目标：弯刀 → 验证目标 1+10=11 未命中；共造成 0 点伤害。
...
| 动作 | 多重攻击：投掷火焰 ×2 / `multiattack-hurl-flame` | headless | multiattack | 通过 | complete-sequence |
...
- [本怪物字段与结果证据](../evidence/efreeti.json)
- [总索引与运行说明](../README.md)
