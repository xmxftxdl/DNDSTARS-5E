...
规则 ID：`srd-5.1:darkmantle`。结论：**本次已覆盖范围通过**。
...
通过：碾压。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 314/320。

> 暗幕怪 使用碾压攻击 验证目标：碾压 → 验证目标 7+5=12 命中；共造成 6 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 碾压 / `crush` | headless | weapon-attack | 通过 | miss、hit、critical |
| 动作 | 黑暗灵光 / `darkness-aura` | headless | persistent-area | 通过 | minimum、maximum、no-resource |
...
| --- | --- | --- | --- | --- |
| 回声定位 | dm-adjudication | 无结构化处理器 | DM 裁定 / 情境判断 | — |
...
- [本怪物字段与结果证据](../evidence/darkmantle.json)
- [总索引与运行说明](../README.md)
