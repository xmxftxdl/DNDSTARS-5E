...
规则 ID：`srd-5.1:magma-mephit`。结论：**本次已覆盖范围通过**。
...
通过：双爪。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 314/320。

> 岩浆魔蝠 使用双爪攻击 验证目标：双爪 → 验证目标 15+3=18 命中；共造成 6 点伤害。
...
| --- | --- | --- | --- | --- |
| 死亡爆裂 | headless | death-area-saving-throw | 通过 | failed-save、successful-save、outside-radius、replay-rejected |
...
- [本怪物字段与结果证据](../evidence/magma-mephit.json)
- [总索引与运行说明](../README.md)
