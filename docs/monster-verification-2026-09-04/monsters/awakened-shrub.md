...
规则 ID：`srd-5.1:awakened-shrub`。结论：**本次已覆盖范围通过**。
...
通过：撕抓。已验证真实 DM 控制台点击、选择目标、动作消耗、玩家日志同步及玩家先攻栏 HP。HP 320 → 320/320。

> 觉醒灌木 使用撕抓攻击 验证目标：撕抓 → 验证目标 4+1=5 未命中；共造成 0 点伤害。
...
| --- | --- | --- | --- | --- | --- |
| 动作 | 撕抓 / `rake` | headless | weapon-attack | 通过 | miss、hit、critical |
...
- [本怪物字段与结果证据](../evidence/awakened-shrub.json)
- [总索引与运行说明](../README.md)
