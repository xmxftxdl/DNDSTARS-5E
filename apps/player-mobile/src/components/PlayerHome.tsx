...
export function PlayerHome({ workspace, onOpenMap, onOpenActions, onEndTurn, onDeathSave }: {
  ...
  return <ScrollView contentContainerStyle={styles.content}>
    <View style={styles.hero}>
      <View style={styles.identity}><Portrait source={token?.portraitSource} fallback={character?.avatar || '🧝'} /><View style={{ flex: 1 }}><Text style={styles.eyebrow}>当前冒险者</Text><Text style={styles.title}>{character?.name ?? '尚未绑定角色'}</Text><Text style={styles.meta}>{character ? `${character.charClass} ${character.level}级 · ${character.race}` : '请先在桌面端创建角色'}</Text>{character?.concentrating && <Text style={styles.concentration}>◉ 正在专注</Text>}{!!character?.conditions.length && <Text numberOfLines={2} style={styles.conditions}>{character.conditions.join(' · ')}</Text>}</View></View>
      <View style={styles.vitals}><Metric label="HP" value={`${token?.hp ?? character?.currentHp ?? 0}/${token?.maxHp ?? character?.maxHp ?? 0}`} tone="health" /><Metric label="AC" value={`${character?.ac ?? 0}`} /><Metric label="速度" value={`${character?.speed ?? 0}尺`} /><Metric label="被动察觉" value={`${character?.passivePerception ?? 0}`} /></View>
    ...
    {combat?.active && <View style={[styles.turnCard, isTurn && styles.myTurn]}><View><Text style={styles.turnEyebrow}>第 {combat.round} 轮</Text><Text style={styles.turnTitle}>{isTurn ? '现在是你的回合' : `正在行动：${current?.label || '等待 Host'}`}</Text></View>{isTurn && <Pressable style={styles.endTurn} onPress={onEndTurn}><Text style={styles.endTurnText}>结束回合</Text></Pressable>}
      {isTurn && <View style={styles.economy}><Economy label="动作" value={economy?.action} /><Economy label="附赠" value={economy?.bonusAction} /><Economy label="反应" value={economy?.reaction} /><Economy label="移动" value={economy?.movementFeet} suffix="尺" /></View>}
    ...
    {character && character.currentHp <= 0 && <Pressable style={styles.deathSave} onPress={onDeathSave}><Text style={styles.deathSaveTitle}>进行死亡豁免</Text><Text style={styles.deathSaveMeta}>Host 会记录成功与失败次数</Text></Pressable>}
...
function Metric({ label, value, tone }: { label: string; value: string; tone?: 'health' }) { return <View style={styles.metric}><Text style={styles.metricLabel}>{label}</Text><Text style={[styles.metricValue, tone === 'health' && styles.health]}>{value}</Text></View> }
...
const styles = StyleSheet.create({
  content: { padding: 16, paddingBottom: 45, gap: 14 }, hero: { padding: 18, borderRadius: 22, backgroundColor: colors.elevated, borderWidth: 1, borderColor: colors.border }, identity: { flexDirection: 'row', alignItems: 'center', gap: 12 }, avatar: { width: 60, height: 60, borderRadius: 30, overflow: 'hidden', backgroundColor: colors.primarySoft, alignItems: 'center', justifyContent: 'center' }, avatarImage: { width: '100%', height: '100%' }, avatarText: { fontSize: 31 }, eyebrow: { color: colors.teal, fontSize: 9, fontWeight: '900', letterSpacing: 1 }, title: { color: colors.text, fontSize: 24, fontWeight: '900', marginTop: 3 }, meta: { color: colors.muted, marginTop: 3, fontSize: 10 }, concentration: { color: colors.warning, fontSize: 9, fontWeight: '900', marginTop: 5 }, conditions: { color: colors.danger, fontSize: 9, marginTop: 3 },
  vitals: { flexDirection: 'row', gap: 7, marginTop: 15 }, metric: { flex: 1, paddingVertical: 9, alignItems: 'center', borderRadius: 11, backgroundColor: colors.surface }, metricLabel: { color: colors.muted, fontSize: 8 }, metricValue: { color: colors.text, fontSize: 13, fontWeight: '900', marginTop: 2 }, health: { color: colors.success }, slots: { gap: 7, paddingTop: 11 }, slot: { minWidth: 48, alignItems: 'center', borderRadius: 10, borderWidth: 1, borderColor: colors.primary, padding: 7 }, slotLevel: { color: colors.muted, fontSize: 8 }, slotValue: { color: '#d8ccff', fontWeight: '900', fontSize: 11, marginTop: 2 },
