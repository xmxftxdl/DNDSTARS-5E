  ...
  return (
    <KeyboardAvoidingView style={styles.keyboardRoot} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      >
        ...
        <TextInput testID="auth-server-url" value={serverUrl} onChangeText={onServerUrlChange} autoCapitalize="none" autoCorrect={false} keyboardType="url" style={styles.input} />
        <View style={styles.serverPresets}>
          <Pressable style={styles.serverPreset} onPress={() => onServerUrlChange('https://astraltracevtt.com')}><Text style={styles.serverPresetText}>正式服务器</Text></Pressable>
          <Pressable style={styles.serverPreset} onPress={() => onServerUrlChange('https://staging.astraltracevtt.com')}><Text style={styles.serverPresetText}>测试服务器</Text></Pressable>
        </View>
        {mode === 'login' ? <>
          <Text style={styles.label}>用户名 / 邮箱 / 手机号</Text>
          <TextInput testID="auth-identifier" value={identifier} onChangeText={setIdentifier} autoCapitalize="none" style={styles.input} />
          <Text style={styles.label}>密码</Text>
          <TextInput testID="auth-password" value={password} onChangeText={setPassword} secureTextEntry style={styles.input} />
          <Pressable testID="auth-login" disabled={busy || !identifier || !password} style={[styles.primary, (busy || !identifier || !password) && styles.disabled]} onPress={() => void onLogin(identifier, password)}><Text style={styles.primaryText}>{busy ? '登录中…' : '登录并恢复冒险'}</Text></Pressable>
        </> : <>
...
const styles = StyleSheet.create({
  ...
  serverPresets: { flexDirection: 'row', gap: 8, marginTop: 8 }, serverPreset: { flex: 1, paddingVertical: 8, alignItems: 'center', borderRadius: 10, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.background }, serverPresetText: { color: colors.muted, fontSize: 11, fontWeight: '800' },
