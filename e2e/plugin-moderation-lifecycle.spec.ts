...
test('插件人工审核支持通过、举报暂停和拒绝版本', async ({ browser, request }) => {
  ...
  await reporterPage.getByRole('textbox', { name: '搜索扩展内容' }).fill(pluginId)
  await reporterPage.getByRole('button', { name: '查看内容', exact: true }).click()
  const published = reporterPage.locator('article').filter({ hasText: pluginId })
  await expect(published).toBeVisible()
  await published.getByRole('button', { name: '举报' }).click()
  await reporterPage.getByTestId('app-dialog-input').fill('该版本需要安全复核')
  await reporterPage.getByTestId('app-dialog-confirm').click()
  ...
  await adminPage.getByTestId('app-dialog-input').fill('等待发布者处理安全报告')
  await adminPage.getByTestId('app-dialog-confirm').click()
  ...
  await adminPage.getByTestId('app-dialog-input').fill('版本说明不足')
  await adminPage.getByTestId('app-dialog-confirm').click()
  await expect(adminPage.getByText('审核决定已保存。')).toBeVisible()
