...
describe('right dice presentation drawer', () => {
  it('allows the DM to edit public color dice before continuing', () => {
    const html = renderToStaticMarkup(createElement(DicePresentationOverlays, {
      ...
      secretConfirmation: { id: 'color', label: '虹光喷射·颜色', targetName: '目标', sides: 8, values: [3], visibility: 'public' },
  ...
  it('uses the existing dark Log panel inside the shared opaque tray', () => {
    const html = renderToStaticMarkup(createElement(DicePresentationOverlays, {
      roll: null, diceBoxD20: null, diceBoxRoll: null, rollRequestPreview: null,
      activeRollStatus: null, secretConfirmation: null, isDM: true, dockTab: 'log',
      combatLogPanel: createElement(MapWorkspaceCombatLogPanel, {
        combatLog: [], tokens: [], characters: [],
        onClearCombatLog: () => undefined, onCloseCombatLog: () => undefined,
      }),
      onRollDone: () => undefined, onD20Complete: () => undefined,
    ...
    expect(html).toContain('隐藏战斗记录')
    expect(html).toContain('dice-tray-drawer--backdrop')
    expect(html).not.toContain('combat-log-dock')
  })

  it('offers rerolls only for the matching local free-roll result', () => {
    const roll = { values: [4, 4, 6], sides: 6, bonus: 2, total: 16, label: '自由掷骰', targetName: '明骰' }
    const props = {
      roll, diceBoxD20: null, diceBoxRoll: null, rollRequestPreview: null,
      activeRollStatus: null, secretConfirmation: null, isDM: true,
      dockTab: 'dice' as const,
      onRollDone: () => undefined, onD20Complete: () => undefined,
      onDiceComplete: () => undefined, onPreviewComplete: () => undefined,
      onSecretConfirm: () => undefined,
    ...
    expect(other).not.toContain('全部重投')
  })
  ...
  it('presents local and shared dice rolls from the right-side tray', () => {
    expect(overlaysSource).toContain('<PersistentDiceTray')
    expect(readFileSync(new URL('./PersistentDiceTray.tsx', import.meta.url), 'utf8')).toContain('layout="left-drawer"')
    ...
    expect(css).toMatch(/\.dice-tray-drawer\s*{[^}]*right:\s*0\.75rem;/s)
    expect(css).toMatch(/\.map-combat-right-dock__recall\s*{[^}]*right:\s*0;/s)
    expect(css).toContain('.dice-tray-drawer__controls')
  })

  it('renders the solid result rail above the 3D dice iframe', () => {
    const diceLayer = overlaysSource.indexOf('<PersistentDiceTray')
  ...
  it('hosts the free-roll settings inside the same physical tray', () => {
    ...
    expect(overlaysSource).toContain('const showFreeRollQuickbar = !showFreeRollControls && props.renderFreeRollControls != null')
  ...
  it('waits for the visual engine failsafe instead of truncating a large dice pool at 3.5 seconds', () => {
    expect(DICE_TIMING.ROLL_SETTLED_HOLD_MS).toBeGreaterThanOrEqual(250)
