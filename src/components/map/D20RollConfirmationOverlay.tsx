...
import { d20RollConfirmationTimeoutContribution } from '../../lib/rollConfirmation'

interface D20RollContributionInput {
  featureId: string
  featureLabel: string
...
interface D20RollConfirmationOverlayProps {
  ...
  busy?: boolean
  onContribute: (input: D20RollContributionInput) => void | Promise<void>
...
export default function D20RollConfirmationOverlay({
  ...
  const storedReplacementValues = useMemo(
    () => selectedFeature?.replacementValues ?? [],
    [selectedFeature?.replacementValues],
  )
  const effectiveReplacementValue = storedReplacementValues.length > 0 &&
    !storedReplacementValues.includes(Number(replacementValue))
    ? String(storedReplacementValues[0])
    : replacementValue
  ...
  const hostSettling = interrupt.status === 'rolling'
  const deadline = !isSecretDmRoll && !hostSettling
    ? interrupt.expiresAt ?? interrupt.createdAt + 10_000
    : undefined
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      const result = isDM
        ...
        : playerCharacter
          ? (() => {
              // Once a reroll was committed, a timeout must never replace it
              // with a character-level decline. Non-owner policies are settled
              // by the Host; owner-choice policies retain the original die.
              const contribution = d20RollConfirmationTimeoutContribution({
                contribution: ownContribution,
                rollOptions,
                selectionPolicy: selectedFeature?.selectionPolicy,
                hasEligibleFeature: ownEligibleFeatures.length > 0,
              })
              return contribution ? onContribute(contribution) : undefined
            })()
          : undefined
  ...
  }, [deadline, interrupt.id, isDM, onContinue, onContribute, ownContribution,
    ownEligibleFeatures.length, playerCharacter, rollOptions, selectedFeature?.selectionPolicy])
  ...
  const submit = async (decline = false, selectedIndex?: number) => {
    ...
    const parsedReplacement = Number(effectiveReplacementValue)
            ...
            <>
              {ownContribution && (
                <p className="rounded-lg border border-emerald-400/25 bg-emerald-500/10 p-2 text-xs text-emerald-100" data-testid="d20-player-decision-sent">
                  {hostSettling ? '第二颗 d20 已提交，正在结算…' : ownContribution.kind === 'decline-d20' ? '已选择不更改点数，Host 正在继续结算。' : requiresResultChoice ? 'Host 已完成额外投骰，请选择最终采用的结果。' : `已选择「${ownContribution.featureLabel}」，Host 正在自动结算。`}
                </p>
              )}
              ...
              <div className="space-y-1.5" data-testid="d20-feature-options" role="radiogroup" aria-label="可用投骰特性">
                ...
                {ownEligibleFeatures.map((feature) => {
                  ...
                  return <button key={feature.featureId} type="button" role="radio" aria-checked={selected}
                    disabled={hostSettling || requiresResultChoice} onClick={() => setSelectedFeatureId(feature.featureId)}
              ...
              {selectedFeature?.modifierKind !== 'adjust-d20' && selectedFeature?.modifierKind !== 'choice-reroll' && (
                <label className="block space-y-1"><span className="text-xs text-slate-400">替换点数</span>
                  {storedReplacementValues.length > 0 ? (
                    <div className="grid grid-cols-4 gap-2" data-testid="d20-stored-replacement-values">
                      {storedReplacementValues.map((value, index) => (
                        <button key={`${index}:${value}`} type="button"
                          ...
                          className={`rounded-xl border px-2 py-2.5 text-xl font-black tabular-nums ${Number(effectiveReplacementValue) === value
                    ...
                    <input type="number" min={1} max={20} value={effectiveReplacementValue}
              ...
              <div className="grid grid-cols-2 gap-2">
                <button type="button" disabled={hostSettling || submitting || requiresResultChoice} onClick={() => void submit(true)} data-testid="d20-roll-decline"
                ...
                <button type="button" disabled={hostSettling || submitting || !selectedFeature || requiresResultChoice} onClick={() => void submit()} data-testid="d20-roll-contribute"
                  ...
                  {hostSettling ? '结算中…' : submitting ? '提交中…' : `使用${selectedFeature?.featureLabel ?? '特性'}`}
                </button>
              </div>
            </>
          )}
