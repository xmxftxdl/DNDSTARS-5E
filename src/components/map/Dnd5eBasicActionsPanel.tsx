...
export default function Dnd5eBasicActionsPanel({
  ...
  const targetRelation = kind === 'help' && helpKind === 'ability-check' ? 'ally'
    : kind === 'help' || kind === 'grapple' || kind === 'shove' || kind === 'escape-grapple' ? 'enemy'
      : 'none'
  ...
  return (
    <section className="rounded-xl border border-white/10 bg-void-900/45 p-4">
      ...
      <p className="mt-2 text-[11px] leading-5 text-slate-500">准备动作先登记触发条件；触发时消耗反应。推开会由 DM 权威端检查边界、占位与墙体，并自动移动 5 尺。</p>
    </section>
  )
}
