...
import { sceneWeatherHasAudio, type SceneWeatherKind } from '../../lib/sceneWeather'
...
const WEATHER_LABELS: Readonly<Record<SceneWeatherKind, string>> = {
  none: '无环境效果',
  sunny: '晴天',
  cloudy: '阴天',
  night: '夜晚（仅视觉）',
...
const WEATHER_WITH_DIRECTION = new Set<SceneWeatherKind>([
  'fog', 'rain', 'snow', 'thunderstorm', 'hail', 'blizzard', 'sandstorm', 'wind', 'embers',
])

const WEATHER_WITH_SPEED = new Set<SceneWeatherKind>([
  'sunny', 'cloudy', 'cave', 'fog', 'rain', 'snow', 'thunderstorm', 'hail', 'blizzard',
  'sandstorm', 'wind', 'embers',
])
...
export default function SceneOrchestrationPanel({
  ...
  const sceneSettingsRef = useRef<HTMLElement>(null)
  const interactionPointsRef = useRef<HTMLDivElement>(null)
  const triggerEditorRef = useRef<HTMLElement>(null)
  const actionBuilderRef = useRef<HTMLDivElement>(null)
  ...
  const effectiveBackgroundAudio = resolveSceneAudioPreset(shared, map.id)

  useEffect(() => {
    if (!open || !focusedInteractionPointId) return
    const frame = window.requestAnimationFrame(() => {
      interactionPointsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })
    })
    return () => window.cancelAnimationFrame(frame)
  }, [focusedInteractionPointId, open])
  ...
  return (
    <>
      {open && (
        <aside data-testid="scene-orchestration-panel" className="absolute bottom-4 right-4 top-28 z-[118] flex w-[min(440px,calc(100%-2rem))] flex-col overflow-hidden rounded-2xl border border-violet-300/20 bg-void-950/96 shadow-2xl backdrop-blur-xl">
          <header className="flex items-start gap-3 border-b border-white/8 p-4">
            <div className="min-w-0 flex-1"><h2 className="font-bold text-violet-100">场景编排</h2><p className="mt-1 text-[11px] text-slate-500">DM 权威 · 区域图层可直接选择和移动，编辑不会触发动作</p></div>
          ...
          {!scene ? (
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-4">
              <nav className="sticky top-0 z-20 mb-3 grid grid-cols-3 gap-1 rounded-xl border border-white/8 bg-void-950/95 p-1.5 shadow-lg backdrop-blur-xl">
                <button type="button" onClick={() => sceneSettingsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="rounded-lg px-2 py-2 text-[11px] font-semibold text-slate-400 hover:bg-white/5 hover:text-slate-100">环境</button>
                <button type="button" onClick={() => interactionPointsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="rounded-lg px-2 py-2 text-[11px] font-semibold text-amber-200/75 hover:bg-amber-500/10 hover:text-amber-100">互动点</button>
                <button type="button" data-testid="scene-trigger-editor-shortcut" onClick={() => triggerEditorRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="rounded-lg bg-violet-500/12 px-2 py-2 text-[11px] font-semibold text-violet-200 hover:bg-violet-500/20">入口 / 出口</button>
              </nav>

              <section ref={sceneSettingsRef} className="scroll-mt-14 space-y-3 rounded-xl border border-white/8 bg-white/[0.025] p-3">
                ...
                <div className="rounded-xl border border-cyan-300/10 bg-cyan-500/[0.035] p-3" data-testid="scene-weather-controls">
                  <div className="mb-2 flex items-center gap-2 text-xs font-bold text-cyan-100">
                    <CloudRain className="h-4 w-4" />场景光照与天气
                    {scene.weather.kind !== 'none' && <span className="ml-auto text-[10px] font-medium text-cyan-200/60">玩家端同步</span>}
                  </div>
                  <select
                    value={scene.weather.kind}
                    onChange={(event) => updateScene(scene.id, {
                      weather: { ...scene.weather, kind: event.target.value as SceneWeatherKind },
                    })}
                  ...
                  {scene.weather.kind !== 'none' && (
                    <div className="mt-3 space-y-2.5">
                      <label className="grid grid-cols-[64px_1fr_42px] items-center gap-2 text-[11px] text-slate-400">
                        <span>强度</span>
                        <input
                          type="range"
                          min={0.15}
                          ...
                          onChange={(event) => updateScene(scene.id, {
                            weather: { ...scene.weather, intensity: Number(event.target.value) },
                          })}
                        />
                        <span className="text-right tabular-nums">{Math.round(scene.weather.intensity * 100)}%</span>
                      </label>
                      {WEATHER_WITH_DIRECTION.has(scene.weather.kind) && (
                        <label className="grid grid-cols-[64px_1fr_42px] items-center gap-2 text-[11px] text-slate-400">
                          <span>风向偏角</span>
                          <input
                            type="range"
                            ...
                            max={70}
                            step={2}
                            value={scene.weather.windAngleDegrees}
                            onChange={(event) => updateScene(scene.id, {
                              weather: { ...scene.weather, windAngleDegrees: Number(event.target.value) },
                            })}
                          />
                          <span className="text-right tabular-nums">{scene.weather.windAngleDegrees}°</span>
                        </label>
                      )}
                      {WEATHER_WITH_SPEED.has(scene.weather.kind) && (
                        <label className="grid grid-cols-[64px_1fr_42px] items-center gap-2 text-[11px] text-slate-400">
                          <span>{scene.weather.kind === 'sunny' || scene.weather.kind === 'cloudy' || scene.weather.kind === 'cave' ? '变化速度' : '速度'}</span>
                          <input
                            type="range"
                            min={0.35}
                            max={2.5}
                            step={0.05}
                            value={scene.weather.speed}
                            onChange={(event) => updateScene(scene.id, {
                              weather: { ...scene.weather, speed: Number(event.target.value) },
                            })}
                          />
                          <span className="text-right tabular-nums">{scene.weather.speed.toFixed(1)}×</span>
                        </label>
                      )}
                      {sceneWeatherHasAudio(scene.weather.kind) && (
                        <div
                          className="rounded-lg border border-sky-300/10 bg-sky-500/[0.04] p-2"
                          data-testid="scene-weather-sound-controls"
                        >
                          <label className="flex items-center gap-2 text-[11px] text-sky-100">
                            <Volume2 className="h-3.5 w-3.5" />
                            <input
                              type="checkbox"
                              checked={scene.weather.soundEnabled}
                              onChange={(event) => updateScene(scene.id, {
                                weather: { ...scene.weather, soundEnabled: event.target.checked },
                              })}
                            />
                            环境音效
                            <span className="ml-auto text-[10px] text-slate-500">首次交互后播放</span>
                          </label>
                          {scene.weather.soundEnabled && (
                            <label className="mt-2 grid grid-cols-[64px_1fr_42px] items-center gap-2 text-[11px] text-slate-400">
                              <span>音量</span>
                              <input
                                type="range"
                                min={0}
                                max={1}
                                step={0.05}
                                value={scene.weather.soundVolume}
                                onChange={(event) => updateScene(scene.id, {
                                  weather: { ...scene.weather, soundVolume: Number(event.target.value) },
                                })}
                              />
                              <span className="text-right tabular-nums">{Math.round(scene.weather.soundVolume * 100)}%</span>
                            </label>
                          )}
                        </div>
                      )}
                      {scene.weather.kind === 'night' ? (
                        <p className="rounded-lg border border-indigo-300/10 bg-indigo-500/[0.05] px-2 py-1.5 text-[10px] leading-4 text-indigo-200/65">
                          夜晚目前只改变画面光色；不会自动启用黑暗视觉、照明范围或攻击劣势规则。
                        </p>
                      ) : (
                        <p className="text-[10px] leading-4 text-slate-600">环境效果使用独立视觉层，不参与视线、移动或 Headless 战斗结算。</p>
                      )}
                    </div>
                  )}
                </div>
                <div className="rounded-xl border border-violet-300/10 bg-violet-500/[0.035] p-3">
                  <div className="mb-2 text-xs font-bold text-violet-100">当前地图音乐</div>
                  <div className="grid grid-cols-2 gap-2">
                    <select value={scene.backgroundAudioMode} onChange={(event) => updateScene(scene.id, { backgroundAudioMode: event.target.value as typeof scene.backgroundAudioMode })} className={fieldClass()}>
                      <option value="inherit">继承全局音乐</option><option value="override">使用地图专属音乐</option><option value="silent">此地图静音</option>
                    </select>
                    {scene.backgroundAudioMode === 'override'
                      ? <select value={scene.backgroundAudioId ?? ''} onChange={(event) => updateScene(scene.id, { backgroundAudioId: event.target.value || undefined })} className={fieldClass()}><option value="">选择地图音乐</option>{audioLibrary.assets.map((asset) => <option key={asset.id} value={asset.id}>{asset.kind === 'music' ? '音乐' : asset.kind === 'ambience' ? '环境' : '音效'} · {asset.name}</option>)}</select>
                      : <div className="flex items-center rounded-lg border border-white/8 px-2.5 text-[11px] text-slate-500">{scene.backgroundAudioMode === 'silent' ? '进入地图时停止自动音乐' : effectiveBackgroundAudio.assetId ? '使用已配置的全局曲目' : '尚未配置全局曲目'}</div>}
                  </div>
                  {scene.backgroundAudioMode === 'override' && <div className="mt-2 space-y-2">
                    <label className="flex items-center gap-2 text-[11px] text-slate-400"><span>房间音量</span><input type="range" min={0} max={1} step={0.05} value={scene.backgroundAudioVolume} onChange={(event) => updateScene(scene.id, { backgroundAudioVolume: Number(event.target.value) })} className="min-w-0 flex-1" /><span>{Math.round(scene.backgroundAudioVolume * 100)}%</span></label>
                    <div className="flex gap-4 text-[11px] text-slate-400"><label className="flex items-center gap-1.5"><input type="checkbox" checked={scene.backgroundAudioAutoPlay} onChange={(event) => updateScene(scene.id, { backgroundAudioAutoPlay: event.target.checked })} />进入地图自动播放</label><label className="flex items-center gap-1.5"><input type="checkbox" checked={scene.backgroundAudioLoop} onChange={(event) => updateScene(scene.id, { backgroundAudioLoop: event.target.checked })} />循环</label></div>
                  </div>}
                  <div className="mt-2 grid grid-cols-[1fr_1fr_auto] gap-2"><select value={scene.backgroundCue} onChange={(event) => updateScene(scene.id, { backgroundCue: event.target.value as SceneAudioCue })} className={fieldClass()}>{Object.entries(AUDIO_LABELS).map(([value, label]) => <option key={value} value={value}>提示音 · {label}</option>)}</select><button type="button" disabled={!effectiveBackgroundAudio.assetId && scene.backgroundCue === 'none'} onClick={onPlayBackground} className="flex items-center justify-center gap-1 rounded-lg border border-violet-300/20 px-3 py-2 text-xs text-violet-200 disabled:opacity-30"><Volume2 className="h-4 w-4" />播放当前预设</button><button type="button" disabled={audioPlayback.status === 'stopped'} onClick={onStopAudio} className="rounded-lg border border-white/10 px-3 py-2 text-xs text-slate-400 disabled:opacity-30">停止</button></div>
                </div>
              ...
              <div ref={interactionPointsRef} className="scroll-mt-14">
                <SceneInteractionPointsEditor
                  key={`${scene.id}:${focusedInteractionPointId ?? 'default'}`}
                  map={map}
                  scene={scene}
                  combatActive={combatActive}
                  requestedSelectedId={focusedInteractionPointId}
              ...
              <section ref={triggerEditorRef} className="mt-4 scroll-mt-14">
                <div className="flex items-center justify-between"><h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">入口 / 出口与触发区域</h3><button type="button" onClick={() => { const id = addTrigger(scene.id, { kind: 'circle', x: map.width / 2, y: map.height / 2, radius: Math.max(30, map.gridSize) }); setSelectedTriggerId(id) }} className="flex items-center gap-1 text-xs text-violet-300"><Plus className="h-3.5 w-3.5" />添加入口</button></div>
              ...
              {selectedTrigger && <section className="mt-3 rounded-xl border border-white/8 bg-black/15 p-3">
                ...
                <p className="mt-2 text-[10px] leading-4 text-cyan-200/55">关闭绘制工具时，可直接在地图上拖动此入口区域；尺寸不变并自动限制在地图内。</p>
                ...
                <div className="mt-4 border-t border-white/8 pt-3"><div className="flex items-center justify-between gap-2"><h4 className="text-xs font-bold text-slate-300">动作序列</h4><div className="flex items-center gap-2"><button type="button" data-testid="scene-configure-teleport" onClick={() => { setActionKind('teleport'); requestAnimationFrame(() => actionBuilderRef.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' })) }} className="rounded-lg border border-violet-300/20 bg-violet-500/10 px-2 py-1.5 text-[11px] font-semibold text-violet-100">+ 设置传送出口</button><button type="button" onClick={() => setPreview((value) => !value)} className="flex items-center gap-1 text-[11px] text-cyan-300"><Eye className="h-3.5 w-3.5" />{preview ? '关闭预览' : '预览'}</button></div></div>{selectedTrigger.actions.length === 0 ? <p className="mt-3 text-center text-xs text-slate-600">尚未添加动作</p> : <ol className="mt-2 space-y-1">{selectedTrigger.actions.map((action, index) => <li key={action.id} className="flex items-center gap-2 rounded-lg border border-white/6 bg-white/[0.02] px-2 py-2 text-xs"><span className="w-5 text-center text-slate-600">{index + 1}</span><span className="min-w-0 flex-1 truncate text-slate-300">{sceneActionSummary(action)}</span><button type="button" onClick={() => removeAction(scene.id, selectedTrigger.id, action.id)} className="text-slate-600 hover:text-red-300"><X className="h-3.5 w-3.5" /></button></li>)}</ol>}{preview && <div className="mt-2 rounded-lg border border-cyan-300/15 bg-cyan-500/[0.05] p-2 text-[11px] leading-5 text-cyan-100/80">触发后将按上方顺序执行；暂停模式下动作进入队列，可逐步执行。消息、讲义、日志、检定与已开始的战斗不可撤销。</div>}</div>

                <div ref={actionBuilderRef} className="scroll-mt-14">
                  <ActionBuilder
                    ...
                    maps={maps} currentMapId={map.id} targetMapId={targetMapId} setTargetMapId={setTargetMapId} targetX={targetX} setTargetX={setTargetX} targetY={targetY} setTargetY={setTargetY}
...
interface ActionBuilderProps {
  ...
  maps: BattleMap[]; currentMapId: string; targetMapId: string; setTargetMapId: (value: string) => void; targetX: number; setTargetX: (value: number) => void; targetY: number; setTargetY: (value: number) => void
...
function ActionBuilder(props: ActionBuilderProps) {
  ...
  return <div className="mt-4 rounded-xl border border-violet-300/12 bg-violet-500/[0.035] p-3"><div className="mb-2 text-[10px] font-bold uppercase tracking-wider text-violet-200/60">新增触发动作</div><div className="flex items-center gap-2"><select aria-label="触发动作类型" value={props.kind} onChange={(event) => { props.setKind(event.target.value as ActionKind); props.setPrimary(''); props.setSecondary('') }} className={`${input} flex-1`}>{Object.entries(ACTION_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><button type="button" onClick={props.onAdd} className="flex shrink-0 items-center gap-1 rounded-lg bg-violet-500 px-3 py-2 text-xs font-bold text-white"><Plus className="h-3.5 w-3.5" />{props.kind === 'teleport' ? '添加出口' : '加入'}</button></div><div className="mt-2 space-y-2">
    ...
    {props.kind === 'teleport' && <div data-testid="scene-teleport-action-fields" className="space-y-2 rounded-lg border border-violet-300/15 bg-violet-500/[0.06] p-2.5"><label className="block text-[11px] font-semibold text-violet-100">目标地图 / 楼层<select aria-label="目标地图/楼层" value={props.targetMapId} onChange={(event) => props.setTargetMapId(event.target.value)} className={`mt-1.5 ${input}`}><option value="">选择目标地图/楼层…</option>{props.maps.map((map) => <option key={map.id} value={map.id}>{map.name}{map.id === props.currentMapId ? '（当前地图）' : ''}</option>)}</select></label><div className="grid grid-cols-2 gap-2"><label className="text-[10px] text-slate-500">初始出口 X<input aria-label="初始出口 X" type="number" value={props.targetX} onChange={(event) => props.setTargetX(Number(event.target.value) || 0)} className={`mt-1 ${input}`} /></label><label className="text-[10px] text-slate-500">初始出口 Y<input aria-label="初始出口 Y" type="number" value={props.targetY} onChange={(event) => props.setTargetY(Number(event.target.value) || 0)} className={`mt-1 ${input}`} /></label></div><p className="text-[10px] leading-4 text-violet-200/55">点击“添加出口”后，切换到目标地图并打开场景编排，可直接拖动紫色“出口”标记调整落点。</p></div>}
...
function SceneGlobalAudioSettings() {
  const preset = useSceneOrchestrationStore((state) => state.shared.globalAudio)
  const update = useSceneOrchestrationStore((state) => state.updateGlobalAudio)
  ...
  return (
    <details className="rounded-xl border border-violet-300/12 bg-violet-500/[0.035] p-3">
      <summary className="cursor-pointer text-xs font-bold text-violet-100"><span className="inline-flex items-center gap-2"><Volume2 className="h-4 w-4" />全局场景音乐{preset.assetId ? ' · 已配置' : ''}</span></summary>
      <div className="mt-3 space-y-2">
        <select value={preset.assetId ?? ''} onChange={(event) => update({ assetId: event.target.value || undefined })} className={fieldClass()}>
          <option value="">不设置全局音乐</option>
          {library.assets.map((asset) => <option key={asset.id} value={asset.id}>{asset.kind === 'music' ? '音乐' : asset.kind === 'ambience' ? '环境' : '音效'} · {asset.name}</option>)}
        </select>
        <label className="flex items-center gap-2 text-[11px] text-slate-400"><span>房间音量</span><input type="range" min={0} max={1} step={0.05} value={preset.volume} onChange={(event) => update({ volume: Number(event.target.value) })} className="min-w-0 flex-1" /><span>{Math.round(preset.volume * 100)}%</span></label>
        <div className="flex flex-wrap gap-4 text-[11px] text-slate-400"><label className="flex items-center gap-1.5"><input type="checkbox" checked={preset.autoPlay} disabled={!preset.assetId} onChange={(event) => update({ autoPlay: event.target.checked })} />进入房间/地图时自动播放</label><label className="flex items-center gap-1.5"><input type="checkbox" checked={preset.loop} onChange={(event) => update({ loop: event.target.checked })} />循环</label></div>
        <div className="flex gap-2"><button type="button" disabled={!preset.assetId} onClick={() => preset.assetId && void control({ operation: 'play', assetId: preset.assetId, loop: preset.loop, volume: preset.volume })} className="flex items-center gap-1 rounded-lg border border-violet-300/20 px-3 py-2 text-xs text-violet-200 disabled:opacity-30"><Play className="h-3.5 w-3.5" />全房间播放</button><button type="button" disabled={playback.status === 'stopped'} onClick={() => void control({ operation: playback.status === 'paused' ? 'resume' : 'pause' })} className="rounded-lg border border-white/10 px-3 py-2 text-xs text-slate-400 disabled:opacity-30">{playback.status === 'paused' ? '继续' : '暂停'}</button><button type="button" disabled={playback.status === 'stopped'} onClick={() => void control({ operation: 'stop' })} className="rounded-lg border border-white/10 px-3 py-2 text-xs text-slate-400 disabled:opacity-30">停止</button></div>
        <p className="text-[10px] leading-4 text-slate-600">未绑定地图；地图选择“继承全局音乐”时会继续使用该曲目，同一首不会因切图重新开始。</p>
      </div>
    </details>
  )
}
