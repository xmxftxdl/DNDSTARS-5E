...
export default function GroupAbilityCheckSystem() {
  ...
  const createCheck = () => execute(async () => {
    await mutate({
      operation: 'create', label, selection: selection as `ability:${typeof selectedDefinition.ability}` | `skill:${string}`,
      dc, mode, allowPassiveFallback: passiveFallback,
              ...
              <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.8fr)]">
                <section className="space-y-4 rounded-2xl border border-white/8 bg-white/[0.025] p-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_90px_120px]">
                    <label className="text-xs text-slate-500">检定项目<select value={selection} onChange={(event) => { const value = event.target.value; setSelection(value); const [kind, key] = value.split(':'); const nextName = kind === 'skill' ? SKILLS.find((entry) => entry.key === key)?.label : ABILITIES.find((entry) => entry.key === key)?.label; setLabel(`全队${nextName ?? '属性'}检定`) }} className="mt-1 w-full rounded-xl border border-white/10 bg-void-900 px-3 py-2.5 text-sm text-slate-200"><optgroup label="属性检定">{ABILITIES.map((ability) => <option key={ability.key} value={`ability:${ability.key}`}>{ability.label}</option>)}</optgroup><optgroup label="技能检定">{SKILLS.map((skill) => <option key={skill.key} value={`skill:${skill.key}`}>{skill.label}（{ABILITIES.find((ability) => ability.key === skill.ability)?.label}）</option>)}</optgroup></select></label>
                  ...
                  <label className="flex items-start gap-2 rounded-xl border border-violet-300/10 bg-violet-500/[0.04] p-3 text-sm text-slate-300"><input type="checkbox" checked={passiveFallback} onChange={(event) => setPassiveFallback(event.target.checked)} className="mt-1" /><span><span className="font-semibold text-violet-100">允许被动值兜底</span><span className="mt-1 block text-xs leading-5 text-slate-500">取掷骰总值与 10＋检定调整值中的较高者；优势／劣势会使被动值 ±5。DM 也可用被动值结算未响应者。</span></span></label>
                </section>
