...
const movements = [
  {
    ...
    pathElevationsFeet: [0, 10, 20],
  },
  {
    tokenId: 'dragged',
    to: { x: 25, y: 15 },
    path: [{ x: 5, y: 15 }, { x: 15, y: 15 }, { x: 25, y: 15 }],
    pathElevationsFeet: [5, 15, 25],
  },
] as const
    ...
    expect(calls).toEqual([
      {
        ...
        pathElevationsFeet: movements[0].pathElevationsFeet,
      },
      {
        tokenId: 'dragged', seen: ['source'], path: movements[1].path,
        pathElevationsFeet: movements[1].pathElevationsFeet,
      },
    ])
