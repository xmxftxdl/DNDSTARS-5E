...
export function mapActorTurnStartResolvedKey(token: Token | undefined, subject: Character | undefined): string | undefined {
  // A missing companion marker means its turn still needs settlement; never
  // substitute the subject's marker, which belongs to a different turn slot.
  if (token?.dnd5eSimulacrum) return token.dnd5eCombatState?.turnStartResolvedTurnKey
  return subject?.dnd5eCombatState?.turnStartResolvedTurnKey ?? token?.dnd5eCombatState?.turnStartResolvedTurnKey
}
...
export function projectSimulacrumCharacterForMapDetail(
  ...
  subject = { ...subject, ...structuredClone(simulacrum.subjectProfile ?? {}) }
  ...
  return {
    ...
    concentrating: !!token.dnd5eCombatState?.concentrationSpellId,
