...
export function dnd5eCombatantCanSee(
  ...
  if (state.physicalLineOfSightBlockedByCombatantPair?.[dnd5eDirectedCombatantPairKey(viewerId, targetId)] === true) return false
...
export function dnd5eAttackerIsUnseenForAttack(
  ...
  return dnd5eAttackerIsUnseen(attacker) || !dnd5eCombatantCanSee(state, targetId, attackerId)
}
...
export function dnd5eTargetIsUnseenForAttack(
  ...
  if (dnd5eUnseenTargetImposesDisadvantage(attacker, target)) return true
  if (dnd5eCombatantCanSee(state, attackerId, targetId)) return false
...
export type Dnd5eAction =
  ...
  | { type: 'monster-legendary-action'; actorId: string; actionId: string; rolls: readonly Dnd5eMonsterActionRoll[] }
  | { type: 'monster-adjudicated-action'; actorId: string; actionId: string; legendary?: boolean; effects: readonly Dnd5eMonsterAdjudicatedEffect[] }
  ...
  | { type: 'death-save-turn'; actorId: string; d20: number; blessRoll?: number; baneRoll?: number; bardicInspirationRoll?: number; darkOnesOwnLuckRoll?: number; activeEffectSavingThrows?: readonly Dnd5eActiveEffectSavingThrowRoll[]; turnStartActiveEffectSavingThrows?: readonly Dnd5eActiveEffectSavingThrowRoll[]; nextMonsterRechargeRolls?: readonly Dnd5eMonsterRechargeRoll[] }
  ...
  | Dnd5ePluginAction
  | { type: 'end-turn'; actorId: string; activeEffectSavingThrows?: readonly Dnd5eActiveEffectSavingThrowRoll[]; turnStartActiveEffectSavingThrows?: readonly Dnd5eActiveEffectSavingThrowRoll[]; nextMonsterRechargeRolls?: readonly Dnd5eMonsterRechargeRoll[] }
...
function resolveWeaponAttack(state: Dnd5eHeadlessCombatState, action: Extract<Dnd5eAction, { type: 'attack' | 'opportunity-attack' }>, events: Dnd5eCombatEvent[]): Dnd5eActionResult {
  ...
  if (hit) {
    ...
    try {
      ...
      damageComponents.push({ total: reduceDamageRoll(damage.total), type: action.damage.type })
...
function resolveSpellCast(
  ...
  if (spell.target === 'hostile' && requestedTargetIds.some((targetId) => dnd5eCannotAttackSource(actor, targetId))) {
    return fail(state, events, 'invalid-target')
  }
  if (
  ...
  const freeCastSource = dnd5eFreeSpellCastSource(contextualCaster, spell, action.slotLevel)
  if (spell.level === 0) {
  ...
  const draconicAffinityBonus = dnd5eCombatantClassLevel(actor, 'sorcerer') >= 6 && dnd5eCombatantHasSubclass(actor, 'sorcerer', 'draconic') &&
    draconicDamageType === spell.damageType
    ? Math.max(0, rules.abilityModifier(actor.abilities.cha))
    : 0
  const empoweredEvocationBonus = dnd5eCombatantClassLevel(actor, 'wizard') >= 10 && dnd5eCombatantHasSubclass(actor, 'wizard', 'evocation') &&
    spell.school === '塑能'
...
function resolveDnd5eMonsterTurnStartLifecycle(input: {
  ...
  if (!monster) return (input.action.nextMonsterRechargeRolls?.length ?? 0) === 0
  ...
  if ((monster.legendaryActions?.length ?? 0) > 0) {
    input.actor.classState.monsterLegendaryActionPoints = 3
    input.events.push({ type: 'monster-legendary-actions-restored', actorId: input.actor.id, points: 3 })
  }
