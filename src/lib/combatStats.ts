...
export function formatEquipmentStatLine(item: EquipmentItem): string {
  ...
  if (!rules) return '旧装备数据不可用于 D&D 5e 结算'
  if (rules.kind === 'weapon') {
    ...
    return `${rules.damage.count}d${rules.damage.sides} ${damageTypeLabel(rules.damage.type)}${range}${properties}`
  }
  if (rules.kind === 'shield') return `AC +${rules.armorClassBonus}`
  ...
  return `AC ${rules.baseArmorClass}${dexterity}${extras.length ? ` · ${extras.join(' · ')}` : ''}`
}
