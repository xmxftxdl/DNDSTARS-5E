    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="mb-4 rounded-2xl border border-sky-400/20 bg-sky-500/[0.06] px-4 py-3 text-sm leading-6 text-sky-100">
        <div className="flex items-start gap-2"><ShieldCheck className="mt-1 h-4 w-4 shrink-0" /><p>
          319 个 SRD 5.1 核心法术均已附带中文版详细资料。规则正文完整不代表已自动结算：标有“Headless”的法术才能自动处理伤害、豁免和状态；其他法术仍由 DM 按正文裁定。DM 导入的 JSON 同样只提供资料，不执行 JavaScript，也不会自动获得 Headless 权限。
        </p></div>
      </div>
...
function SpellDetails({ entry, isDM, busy, onRemove }: { entry?: Dnd5eSpellbookEntry; isDM: boolean; busy: boolean; onRemove: (spell: Dnd5eImportedSpell) => void }) {
  ...
  return <section className="glass rounded-2xl p-5">
    ...
    {imported ? <div className="mt-5 rounded-xl border border-white/8 bg-black/10 p-3 text-xs leading-5 text-slate-500">来源：{imported.source.title} · 发布者：{imported.source.publisher} · 许可证：{imported.source.license}<br />ID：{imported.id}</div> : reference ? <div className="mt-5 rounded-xl border border-white/8 bg-black/10 p-3 text-xs leading-5 text-slate-500">中文资料条目：{reference.sourceName}（{reference.sourceEnglishName}）· PDF 第 {reference.sourcePage} 页<br />SRD ID：{entry.id} · 自动结算：{entry.headless ? '已接入 Headless' : '尚未接入，由 DM 裁定'}</div> : <div className="mt-5 text-xs text-slate-600">ID：{entry.id}</div>}
  </section>
}
